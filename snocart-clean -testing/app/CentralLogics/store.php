...
class StoreLogic
{
    public static function get_stores( $zone_id, $filter_data, $type, $store_type, $limit = 10, $offset = 1, $featured=false,$longitude=0,$latitude=0,$filter=null,$rating_count=null)
    {
        ...
        // PERFORMANCE FIX: Batch load category_ids and discount_status for all stores in single queries
        $storeIds = $paginator->pluck('id')->toArray();

        if (!empty($storeIds)) {
            $allCategoryData = DB::table('items')
                ...
                ->groupBy('items.store_id', 'categories.id', 'categories.parent_id')
                ->get()
                ->groupBy('store_id');

            // PERFORMANCE FIX: Batch discount_status check (1 query for all stores)
            $discountedStores = DB::table('items')
            ...
            $paginator->each(function ($store) use ($allCategoryData, $discountedStores) {
                $categoryData = $allCategoryData->get($store->id, collect());
                ...
                $store->discount_status = isset($discountedStores[$store->id]);
                unset($store['items']);
            });
        }
    ...
    public static function get_discounted_stores($zone_id, $limit = 50, $offset = 1, $type = 'all',$longitude=0,$latitude=0,$filter=null,$rating_count=null)
    {
        ...
        // PERFORMANCE FIX: Batch load category_ids and discount_status for all stores
        $storeIds2 = $paginator->pluck('id')->toArray();
        $allCategoryData2 = !empty($storeIds2) ? DB::table('items')
            ->join('categories', 'items.category_id', '=', 'categories.id')
            ->selectRaw('items.store_id, CAST(categories.id AS UNSIGNED) as id, categories.parent_id')
            ->whereIn('items.store_id', $storeIds2)
            ->where('categories.status', 1)
            ->groupBy('items.store_id', 'categories.id', 'categories.parent_id')
            ->get()->groupBy('store_id') : collect([]);
        $discountedStores2 = !empty($storeIds2) ? DB::table('items')
            ->whereIn('store_id', $storeIds2)->where('discount', '>', 0)
            ->distinct()->pluck('store_id')->flip() : collect([]);

        $paginator->each(function ($store) use ($allCategoryData2, $discountedStores2) {
            $data = $allCategoryData2->get($store->id, collect())->toArray();
            $mergedIds = [];
            foreach ($data as $item) {
                $item = (array)$item;
                if ($item['id'] != 0) { $mergedIds[] = $item['id']; }
                if ($item['parent_id'] != 0) { $mergedIds[] = $item['parent_id']; }
            }
            $store->category_ids = array_values(array_unique($mergedIds));
            $store->discount_status = isset($discountedStores2[$store->id]);
            unset($store['items']);
        });
    ...
    public static function search_stores($name, $zone_id, $category_id= null,$limit = 10, $offset = 1, $type = 'all',$longitude=0,$latitude=0,$filter=null,$rating_count=null,$category_ids=null)
    {
        ...
        // PERFORMANCE FIX: Batch load category_ids and discount_status
        $storeIds3 = $paginator->pluck('id')->toArray();
        $allCategoryData3 = !empty($storeIds3) ? DB::table('items')
            ->join('categories', 'items.category_id', '=', 'categories.id')
            ->selectRaw('items.store_id, CAST(categories.id AS UNSIGNED) as id, categories.parent_id')
            ->whereIn('items.store_id', $storeIds3)
            ->where('categories.status', 1)
            ->groupBy('items.store_id', 'categories.id', 'categories.parent_id')
            ->get()->groupBy('store_id') : collect([]);
        $discountedStores3 = !empty($storeIds3) ? DB::table('items')
            ->whereIn('store_id', $storeIds3)->where('discount', '>', 0)
            ->distinct()->pluck('store_id')->flip() : collect([]);

        $paginator->each(function ($store) use ($allCategoryData3, $discountedStores3) {
            $data = $allCategoryData3->get($store->id, collect())->toArray();
            $mergedIds = [];
            foreach ($data as $item) {
                $item = (array)$item;
                if ($item['id'] != 0) { $mergedIds[] = $item['id']; }
            ...
            $store->category_ids = array_values(array_unique($mergedIds));
            $store->discount_status = isset($discountedStores3[$store->id]);
            unset($store['items']);
        });
    ...
    public static function get_recommended_stores($zone_id, $limit = 50, $offset = 1, $type = 'all',$longitude=0,$latitude=0)
    {
        ...
        if($recommended_store_default_status == '1') {
            $query = $query->orderBy('open', 'desc')->orderBy('distance');
        }else{
