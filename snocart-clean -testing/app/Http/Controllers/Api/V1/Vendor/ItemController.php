...
class ItemController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        $p->image = $request->has('image') ? Helpers::update('product/', $p->image, 'png', $request->file('image'), $p->id) : $p->image;
        ...
        foreach ($p['images'] as $img) {
            if (!in_array($img, json_decode($request->images, true))) {
                // Pass item ID to check if other items use this image before deleting
                Helpers::check_and_delete('product/' , $img, $p->id);
...
public function search(Request $request)
{
    ...
    if (!isset($request['vendor']) || !isset($request['vendor']->stores[0])) {
        return response()->json([
            'errors' => [
                ['code' => 'unauthorized', 'message' => translate('messages.vendor_not_found')]
            ]
        ], 404);
    }

    // Get search term
    $search_term = $request->input('name') ?? $request->input('search') ?? $request->input('query');
    ...
    try {
        ...
        // Use the Helpers method to format the items properly
        $formattedItems = Helpers::product_data_formatting($items, true, false, app()->getLocale());

        $data = [
            'items' => $formattedItems,
            'total_size' => count($formattedItems),
        ];

        return response()->json($data, 200);
        
    } catch (\Exception $e) {
        \Log::error('Vendor item search error: ' . $e->getMessage());
        \Log::error('Stack trace: ' . $e->getTraceAsString());
        
        return response()->json([
            'errors' => [
                ['code' => 'error', 'message' => 'Search failed: ' . $e->getMessage()]
            ]
        ], 500);
    }
}
...
public function toggle_stock_status(Request $request)
{
    $validator = Validator::make($request->all(), [
    ...
    // Use the same pattern as other methods
    $store_id = $request['vendor']->stores[0]->id;

    $item = Item::where('id', $request->product_id)
    ...
    if (!$item) {
        return response()->json([
            'errors' => [
                ['code' => 'not_found', 'message' => translate('messages.item_not_found')]
            ]
        ], 404);
    }

    // Toggle stock: if stock > 0, set to 0 (out of stock), otherwise set to 1 (in stock)
    $item->stock = $item->stock > 0 ? 0 : 1;
    $item->save();
    
    return response()->json([
        'message' => $item->stock == 0
            ? translate('messages.item_marked_as_out_of_stock')
            : translate('messages.item_marked_as_in_stock'),
        'item' => $item,
        'success' => true
    ], 200);
}

public function quick_stock_update(Request $request)
{
    $validator = Validator::make($request->all(), [
        'product_id' => 'required|exists:items,id',
    ...
    return response()->json([
        'message' => translate('messages.stock_updated_successfully'),
        'item' => $item,
        'success' => true
    ], 200);
}
...
public function stock_update(Request $request)
{
    $validator = Validator::make($request->all(), [
        ...
        'price' => 'sometimes|numeric|min:0.01', // Add price validation
    ]);
    ...
    // ADD THIS: Handle main product price update
    if ($request->has('price')) {
        $new_price = $request->price;
        
        // Validate discount doesn't exceed new price
        if ($product->discount_type == 'percent') {
            $discount_amount = ($new_price / 100) * $product->discount;
        } else {
            $discount_amount = $product->discount;
        }
        
        if ($new_price <= $discount_amount) {
            return response()->json([
                'errors' => [
                    ['code' => 'price-001', 'message' => translate('messages.discount_can_not_be_more_than_or_equal')]
                ]
            ], 403);
        }
    ...
    $message = translate('messages.Stock_updated_successfully');
    if ($request->has('price')) {
        $message = translate('messages.Stock_and_price_updated_successfully');
    }
    ...
    public function stock_limit_list(Request $request)
    {
        ...
        // Get sales count for each item using raw SQL (last 6 months for performance)
        $sales_counts = collect(DB::select("
            ...
            AND o.store_id = ?
            GROUP BY item_id
        ", [$request['vendor']?->stores[0]?->id]))->keyBy('item_id');

        // Get top selling items to mark as best sellers (top 10% of items)
        $top_selling_threshold = $sales_counts->max('total_sold') * 0.1; // Top 10% of sales
        $top_selling_items = $sales_counts->filter(function ($item) use ($top_selling_threshold) {
            return $item->total_sold >= $top_selling_threshold;
        })->pluck('item_id');
        ...
        // Left join with sales data to sort by popularity (last 6 months)
        $items = $items->leftJoinSub("
                SELECT item_id, SUM(quantity) as total_sold
                FROM order_details od
                INNER JOIN orders o ON od.order_id = o.id
                WHERE o.order_status = 'delivered'
                AND o.created_at >= DATE_SUB(NOW(), INTERVAL 6 MONTH)
                AND o.store_id = " . (int)$request['vendor']?->stores[0]?->id . "
                GROUP BY item_id
            ", 'sales', 'items.id', '=', 'sales.item_id')
            ->select('items.*', DB::raw('COALESCE(sales.total_sold, 0) as total_sales'))
            ->orderBy('total_sales', 'DESC') // Most popular first
            ->orderBy('stock', 'ASC') // Then by stock level (lowest first)
            ->paginate($limit, ['*'], 'page', $offset);

        // Add best_seller flag and sales count to items
        $items->getCollection()->transform(function ($item) use ($top_selling_items, $sales_counts) {
            $item->is_best_seller = $top_selling_items->contains($item->id);
            $item->total_sold = $sales_counts->get($item->id)?->total_sold ?? 0;
            return $item;
        });
