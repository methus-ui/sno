...
class CustomerAuthController extends Controller
{
    public function verify_phone_or_email(Request $request)
    {
        ...
        if($user && $request->login_type== 'manual')
        {
            ...
            /**
             * Security Fix (2026-01-28): Test OTP bypass now requires BOTH conditions:
             * 1. APP_MODE must be 'test' (not 'live' or 'production')
             * 2. APP_DEBUG must be true
             * 3. APP_ENV must NOT be 'production' or 'live'
             *
             * This prevents accidental test mode in production.
             * Rollback: Run storage/security_backups/20260128/ROLLBACK.sh
             */
            $isTestMode = env('APP_MODE') == 'test'
                && env('APP_DEBUG') == true
                && !in_array(env('APP_ENV'), ['production', 'live']);

            if($isTestMode)
            {
                // Log test OTP usage for security audit
                \Log::warning('Test OTP bypass attempted', [
                    'user_id' => $user->id,
                    'ip' => $request->ip(),
        ...
        if($request->login_type== 'otp'){
            ...
            if($data){
                if($user && $user->is_phone_verified == 0 && $user->is_from_pos == 0){
                    ...
                    // Check if user already exists (duplicate phone)
                    if (!$user) {
                        $user = new User();
                        ...
                        // User exists but doesn't match previous conditions - just update verification
                        $user->is_phone_verified = 1;
                        if (!$user->login_medium) {
                            $user->login_medium = 'otp';
                        }
                        $user->save();
                    }
    ...
    private function check_guest_cart($user, $guest_id){
        if($guest_id  && isset($user->id)){
            ...
            // ✅ FIX: Use whereHasMorph for morphTo relationships to avoid SQL syntax errors
            Cart::where('user_id', $user->id)
                ->whereHasMorph('item', [\App\Models\Item::class, \App\Models\ItemCampaign::class], function ($query) use ($userStoreIds) {
