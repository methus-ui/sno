...
class AdvertisementController extends Controller
{
    ...
    public function store(AdvertisementStoreRequest $request)
    {
        ...
        // Handle video: upload or Instagram reel
        if ($request->advertisement_type == 'video_promotion') {
            if ($request->video_source == 'instagram_reel' && $request->instagram_reel_url) {
                $advertisement->instagram_reel_url = $request->instagram_reel_url;
                $advertisement->video_source = 'instagram_reel';
            } elseif ($request->video_source == 'instagram_url' && $request->instagram_manual_url) {
                $advertisement->instagram_reel_url = $request->instagram_manual_url;
                $advertisement->video_source = 'instagram_url';
            } elseif ($request->has('video_attachment')) {
                $advertisement->video_attachment = Helpers::upload(dir: 'advertisement/', format:$request->file('video_attachment')->getClientOriginalExtension(), image:$request->file('video_attachment'));
                $advertisement->video_source = 'upload';
            }
        }
    ...
    public function update(AdvertisementUpdateRequest $request, Advertisement $advertisement)
    {
        ...
        // Handle video update: upload or Instagram reel
        if ($request->advertisement_type == 'video_promotion') {
            if ($request->video_source == 'instagram_reel' && $request->instagram_reel_url) {
                $advertisement->instagram_reel_url = $request->instagram_reel_url;
                $advertisement->video_source = 'instagram_reel';
                // Clear uploaded video if switching to Instagram
                if ($advertisement->video_attachment) {
                    Helpers::check_and_delete('advertisement/', $advertisement->video_attachment);
                    $advertisement->video_attachment = null;
                }
            } elseif ($request->video_source == 'instagram_url' && $request->instagram_manual_url) {
                $advertisement->instagram_reel_url = $request->instagram_manual_url;
                $advertisement->video_source = 'instagram_url';
                ...
                $advertisement->video_attachment = Helpers::update(dir:'advertisement/', old_image: $advertisement->video_attachment, format:$request->file('video_attachment')->getClientOriginalExtension(), image: $request->file('video_attachment'));
                $advertisement->video_source = 'upload';
                // Clear Instagram URL if switching to upload
                $advertisement->instagram_reel_url = null;
            }
        }
    ...
    /**
     * Get Instagram reels for selected store (AJAX endpoint)
     */
    public function getInstagramReels(Request $request)
    {
        try {
            ...
            $result = $instagramService->getStoreReels($storeId);
    ...
    /**
     * Connect Instagram account to store
     */
    public function connectInstagram(Request $request)
    {
        ...
        try {
            $instagramService = new InstagramService();
            $result = $instagramService->connectInstagramAccount(
                $request->store_id,
            ...
            return response()->json([
                'success' => false,
                'message' => 'Error connecting Instagram: ' . $e->getMessage()
            ], 500);
        }
    }
    ...
    public function disconnectInstagram(Request $request)
    {
        ...
        try {
            $instagramService = new InstagramService();
            $result = $instagramService->disconnectInstagramAccount($request->store_id);
     ...
     */
    public function getReelDetails(Request $request)
    {
        $request->validate([
        ...
        try {
            $instagramService = new InstagramService();
            $details = $instagramService->getReelDetailsByUrl($request->reel_url);
            ...
            return response()->json([
                'success' => false,
                'message' => 'Could not fetch reel details'
            ], 404);
