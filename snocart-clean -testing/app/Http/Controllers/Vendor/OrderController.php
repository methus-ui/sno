...
//    }
    ...
    public function edit_order_amount(Request $request)
    {
        ...
        // Store original order amount on first edit
        if (!$order->original_order_amount || $order->original_order_amount == 0) {
            $order->original_order_amount = $order->getOriginal('order_amount');
        }
        $originalTotal = $order->original_order_amount;
        $adjustment = $order->order_amount - $originalTotal;
        ...
        if ($adjustment > 0) {
            if ($isDigitalPayment && $order->user_id) {
            ...
            if ($isDigitalPayment) {
                if ($order->user_id && !$order->wallet_refund_processed) {
                    ...
                    Toastr::success(translate('messages.refund_amount') . ': ' . Helpers::format_currency($refundAmount) . ' ' . translate('messages.added_to_customer_wallet'));
                }
            } else {
                $order->cod_collection_amount = 0;
            }
        ...
        if (config('app.enable_transaction_sync', true)) {
            try {
                \App\Services\OrderTransactionService::updateFromOrderEdit(
                    $order,
                    'vendor',
                    Helpers::get_vendor_id(),
                    [
                        'order_amount_edited' => true,
                        'edited_via' => 'vendor_amount_edit',
            ...
            try {
                \App\Services\OrderTransactionService::updateFromOrderEdit(
                    ...
                    [
                        ...
                        'edited_via' => 'vendor_discount_edit',
                    ]
                );
            } catch (\Exception $e) {
                \Log::error("Transaction sync failed for order {$order->id}: " . $e->getMessage());
    ...
    /**
     * Order View V2 - Vendor panel with bargaining integration
     * NON-BREAKING: Completely separate from v1 details() method
     */
    public function viewV2(Request $request, Order $order)
    {
        // Validate vendor owns this order
        if ($order->store_id !== Helpers::get_store_id()) {
        ...
        $order->load([
            ...
            'customer', 'delivery_man', 'offline_payments',
            'bargainingRequest', 'bargainingAcceptedOffer.offerItems',
        ]);

        // Vendor-specific bargaining perspective
        $bargainingData = null;
        if ($order->is_bargaining_order && $order->bargainingAcceptedOffer) {
            $myOffer = $order->bargainingAcceptedOffer;

            $bargainingData = [
                'request_code' => $order->bargainingRequest->request_code,
                'mode' => $order->bargainingRequest->mode,
                'my_rank' => $myOffer->rank,
                'total_competitors' => $order->bargainingRequest->total_offers_received - 1,
                'fulfillment_percentage' => $myOffer->fulfillment_percentage,
                'items_available' => $myOffer->items_available,
                'items_missing' => $myOffer->items_missing,
                'missing_items_detail' => $myOffer->missing_items_detail,
                'my_notes' => $myOffer->vendor_notes,
                'special_discount_given' => $myOffer->special_discount ?? 0,
                'offer_type' => $myOffer->offer_type,
            ];
        }

        $reasons = \App\Models\OrderCancelReason::where('status', 1)
            ->where('user_type', 'store')->get();

        $transaction = \App\Models\OrderTransaction::where('order_id', $order->id)->first();
        $editHistory = $transaction && $transaction->edit_history
            ? json_decode($transaction->edit_history, true) : [];

        return view('vendor-views.order.order-view-v2', compact(
            'order', 'reasons', 'bargainingData', 'transaction', 'editHistory'
        ));
    }
}
