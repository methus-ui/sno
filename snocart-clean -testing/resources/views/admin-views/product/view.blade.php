            ...
            <div class="d-flex flex-wrap justify-content-between">
                ...
                <div>
                    @if (Config::get('module.current_module_type') != 'food')
                        ...
                        <button type="button" class="btn btn-danger mark-out-of-stock-btn"
                            data-id="{{ $product->id }}"
                            data-name="{{ $product->name }}">
                            <i class="tio-block-outlined"></i> {{ translate('messages.mark_as_out_of_stock') }}
                        </button>
                        <form id="mark-out-of-stock-form" action="{{ route('admin.item.mark-out-of-stock', $product->id) }}" method="POST" style="display:none;">
                            @csrf
                        </form>
                    @endif
...
<script>
    ...
    $('.mark-out-of-stock-btn').on('click', function () {
        const name = $(this).data('name');
        Swal.fire({
            title: '{{ translate('messages.are_you_sure') }}',
            text: '{{ translate('messages.mark_as_out_of_stock_confirmation') }}' + ' "' + name + '"?',
            type: 'warning',
            showCancelButton: true,
            cancelButtonColor: 'default',
            confirmButtonColor: '#FC6A57',
