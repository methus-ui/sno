                            ...
                            <div class="__bg-F8F9FC-card p-0 mt-4">
                                ...
                                <div class="card-body">
                                    <div class="row g-3 align-items-end">
                                        ...
                                        {{-- Tiered Additional Charge Section --}}
                                        <div class="col-12">
                                            @php($tiered_enabled = \App\Models\BusinessSetting::where('key', 'additional_charge_tiered_enabled')->first())
                                            @php($tiered_enabled = $tiered_enabled ? $tiered_enabled->value : 0)
                                            @php($tiers = \App\Models\BusinessSetting::where('key', 'additional_charge_tiers')->first())
                                            @php($tiers = $tiers ? json_decode($tiers->value, true) : [])

                                            <div class="card border mt-3">
                                                <div class="card-header bg-light">
                                                    <div class="d-flex justify-content-between align-items-center">
                                                        <h5 class="mb-0">{{ translate('Tiered Additional Charges') }}</h5>
                                                        <label class="toggle-switch toggle-switch-sm mb-0">
                                                            <input type="checkbox" name="additional_charge_tiered_enabled"
                                                                id="additional_charge_tiered_enabled"
                                                                class="toggle-switch-input"
                                                                value="1"
                                                                {{ $tiered_enabled == 1 ? 'checked' : '' }}
                                                                {{ isset($additional_charge_status) && $additional_charge_status == 1 ? '' : 'disabled' }}>
                                                            <span class="toggle-switch-label text">
                                                                <span class="toggle-switch-indicator"></span>
                                                            </span>
                                                        </label>
                                                    </div>
                                                    <small class="text-muted">{{ translate('Enable different charges based on order amount ranges') }}</small>
                                                </div>
                                                <div class="card-body" id="tiered_charge_section" style="{{ $tiered_enabled == 1 ? '' : 'display:none;' }}">
                                                    <div id="tier-rows">
                                                        @if(!empty($tiers) && is_array($tiers))
                                                            @foreach($tiers as $index => $tier)
                                                            <div class="row tier-row align-items-end mb-3" data-index="{{ $index }}">
                                                                <div class="col-md-3">
                                                                    <label class="form-label">{{ translate('Min Amount') }} ({{ \App\CentralLogics\Helpers::currency_symbol() }})</label>
                                                                    <input type="number" name="tier_min[]" class="form-control"
                                                                        value="{{ $tier['min'] ?? 0 }}" min="0" step="0.01" required>
                                                                </div>
                                                                <div class="col-md-3">
                                                                    <label class="form-label">{{ translate('Max Amount') }} ({{ \App\CentralLogics\Helpers::currency_symbol() }})</label>
                                                                    <input type="number" name="tier_max[]" class="form-control"
                                                                        value="{{ $tier['max'] ?? '' }}" min="0" step="0.01"
                                                                        placeholder="{{ translate('Leave blank for no limit') }}">
                                                                </div>
                                                                <div class="col-md-3">
                                                                    <label class="form-label">{{ translate('Charge') }} ({{ \App\CentralLogics\Helpers::currency_symbol() }})</label>
                                                                    <input type="number" name="tier_charge[]" class="form-control"
                                                                        value="{{ $tier['charge'] ?? 0 }}" min="0" step="0.01" required>
                                                                </div>
                                                                <div class="col-md-3">
                                                                    <button type="button" class="btn btn-danger btn-sm remove-tier" {{ $index == 0 ? 'disabled' : '' }}>
                                                            ...
                                                            @endforeach
                                                        @else
                                                            {{-- Default tier if none exist --}}
                                                            <div class="row tier-row align-items-end mb-3" data-index="0">
                                                                <div class="col-md-3">
                                                                    <label class="form-label">{{ translate('Min Amount') }} ({{ \App\CentralLogics\Helpers::currency_symbol() }})</label>
                                                                    <input type="number" name="tier_min[]" class="form-control" value="0" min="0" step="0.01" required>
                                                                </div>
                                                                <div class="col-md-3">
                                                                    <label class="form-label">{{ translate('Max Amount') }} ({{ \App\CentralLogics\Helpers::currency_symbol() }})</label>
                                                                    <input type="number" name="tier_max[]" class="form-control" value="499.99" min="0" step="0.01" placeholder="{{ translate('Leave blank for no limit') }}">
                                                                </div>
                                                                <div class="col-md-3">
                                                                    <label class="form-label">{{ translate('Charge') }} ({{ \App\CentralLogics\Helpers::currency_symbol() }})</label>
                                                                    <input type="number" name="tier_charge[]" class="form-control" value="25" min="0" step="0.01" required>
                                                                </div>
                                                                <div class="col-md-3">
                                                                    <button type="button" class="btn btn-danger btn-sm remove-tier" disabled>
                                                                        <i class="tio-delete"></i> {{ translate('Remove') }}
                                                                    </button>
                                                                </div>
                                                            </div>
                                                    ...
                                                    <button type="button" class="btn btn-primary btn-sm" id="add-tier">
                                                        <i class="tio-add"></i> {{ translate('Add Tier') }}
                                                    </button>
                                                    <div class="alert alert-info mt-3">
                                                        <i class="tio-info"></i> {{ translate('Example: Set Min=0, Max=499.99, Charge=25 for orders under ₹500. Set Min=500, Max=999.99, Charge=30 for orders ₹500-999. Set Min=1000, Max=(blank), Charge=35 for orders ₹1000+') }}
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
...
<script>
    ...
    <script>
        ...
        // Tiered Additional Charge UI
        $(document).ready(function() {
            // Toggle tiered charge section visibility
            $('#additional_charge_tiered_enabled').on('change', function() {
                if ($(this).is(':checked')) {
                    $('#tiered_charge_section').slideDown();
                } else {
                    $('#tiered_charge_section').slideUp();
                }
            });

            // Disable/enable tiered toggle when main additional charge is off
            $('#additional_charge_status').on('change', function() {
                if ($(this).is(':checked')) {
                    $('#additional_charge_tiered_enabled').prop('disabled', false);
                } else {
                    $('#additional_charge_tiered_enabled').prop('disabled', true).prop('checked', false);
                    $('#tiered_charge_section').slideUp();
                }
            });
            ...
            $('#add-tier').on('click', function() {
                const tierRows = $('#tier-rows .tier-row');
                const newIndex = tierRows.length;

                const newRow = `
                    <div class="row tier-row align-items-end mb-3" data-index="${newIndex}">
                        <div class="col-md-3">
                            <label class="form-label">{{ translate('Min Amount') }} ({{ \App\CentralLogics\Helpers::currency_symbol() }})</label>
                            <input type="number" name="tier_min[]" class="form-control" value="0" min="0" step="0.01" required>
                        </div>
                        <div class="col-md-3">
                            <label class="form-label">{{ translate('Max Amount') }} ({{ \App\CentralLogics\Helpers::currency_symbol() }})</label>
                            <input type="number" name="tier_max[]" class="form-control" value="" min="0" step="0.01" placeholder="{{ translate('Leave blank for no limit') }}">
                        </div>
                        <div class="col-md-3">
                            <label class="form-label">{{ translate('Charge') }} ({{ \App\CentralLogics\Helpers::currency_symbol() }})</label>
                            <input type="number" name="tier_charge[]" class="form-control" value="0" min="0" step="0.01" required>
                        </div>
                        <div class="col-md-3">
            ...
            // Update remove button states (disable first row's remove button)
            function updateRemoveButtons() {
                const tierRows = $('#tier-rows .tier-row');
                tierRows.each(function(index) {
                    const removeBtn = $(this).find('.remove-tier');
                    if (index === 0) {
