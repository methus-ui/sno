...
@section('title', translate('messages.rush_incentives'))
@section('content')
<div class="content container-fluid">
    <div class="page-header">
        <div class="d-flex flex-wrap justify-content-between align-items-center">
            <h1 class="page-header-title"><i class="tio-flash"></i> Rush / Surge Incentives
                <span class="badge badge-soft-dark ml-2">{{ $incentives->total() }}</span>
            </h1>
            <a href="{{ route('admin.transactions.payroll.rush-incentives.active') }}" class="btn btn-warning">
                <i class="tio-flash"></i> Active Rushes
            </a>
        </div>
    </div>

    <div class="card mb-3">
        <div class="card-header"><h5 class="card-header-title">Add Rush Incentive Rule</h5></div>
        <div class="card-body">
            <form method="POST" action="{{ route('admin.transactions.payroll.rush-incentives.store') }}">
                @csrf
                <div class="row g-3">
                    <div class="col-sm-3">
                        <label class="form-label">Title <span class="text-danger">*</span></label>
                        <input type="text" name="title" class="form-control" placeholder="e.g. High Demand Surge" required>
                    </div>
                    <div class="col-sm-2">
                        <label class="form-label">Trigger Type</label>
                        <select name="trigger_type" class="form-control" required>
                            <option value="order_count">Pending Order Count</option>
                            <option value="pending_time">Pending Time</option>
                        </select>
                    </div>
                    <div class="col-sm-2">
                        <label class="form-label">Min Threshold <span class="text-danger">*</span></label>
                        <input type="number" name="min_threshold" class="form-control" required min="1">
                    </div>
                    <div class="col-sm-2">
                        <label class="form-label">Max Threshold</label>
                        <input type="number" name="max_threshold" class="form-control" min="1">
                    </div>
                    <div class="col-sm-2">
            ...
            <div class="table-responsive">
                <table class="table table-borderless table-thead-bordered table-nowrap card-table">
                    <thead class="thead-light">
                        <tr>
                            <th>SL</th><th>Title</th><th>Trigger</th><th>Threshold</th><th>Duration</th><th>Bonus</th><th>Zone</th><th>Status</th><th>Action</th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $inc->title }}</td>
                            <td><span class="badge badge-soft-info">{{ str_replace('_', ' ', $inc->trigger_type) }}</span></td>
                            <td>{{ $inc->min_threshold }}{{ $inc->max_threshold ? ' - '.$inc->max_threshold : '+' }}</td>
                            <td>{{ $inc->duration_minutes }} min</td>
                            ...
                            <td>
                                <form method="POST" action="{{ route('admin.transactions.payroll.rush-incentives.toggle', $inc->id) }}" class="d-inline">
                                    @csrf
                            ...
                            <td>
                                <form method="POST" action="{{ route('admin.transactions.payroll.rush-incentives.delete', $inc->id) }}" class="d-inline">
                                    @csrf @method('DELETE')
