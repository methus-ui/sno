        ...
        <div class="card">
            ...
            <div class="card-header py-2 border-0">
                <div class="search--button-wrapper">
                    ...
                    @php
                        $security_deposit_enabled = \App\Models\BusinessSetting::where('key', 'security_deposit_enabled')->first()?->value == '1';
                        if($security_deposit_enabled) {
                            $paid_count = $deliveryMen->where('security_deposit_status', 'paid')->count();
                            $unpaid_count = $deliveryMen->where('security_deposit_status', 'unpaid')->count();
                        }
                    @endphp
                    @if($security_deposit_enabled)
                        <div class="d-flex gap-2 ml-3">
                            <span class="badge badge-soft-success" data-toggle="tooltip" title="{{ translate('messages.paid_security_deposit') }}">
                                <i class="tio-checkmark-circle"></i> {{ translate('messages.paid') }}: {{$paid_count}}
                            </span>
                            <span class="badge badge-soft-warning" data-toggle="tooltip" title="{{ translate('messages.unpaid_security_deposit') }}">
                                <i class="tio-time"></i> {{ translate('messages.unpaid') }}: {{$unpaid_count}}
                            </span>
                        </div>
                    @endif
                    </h5>
            ...
            <div class="table-responsive datatable-custom">
                <table id="columnSearchDatatable"
                    ...
                    <tr>
                        ...
                        <th class="border-0 text-capitalize">{{translate('messages.security_deposit')}}</th>
                    ...
                    @foreach($deliveryMen as $key=>$dm)
                        <tr>
                            ...
                            <td>
                                @php
                                    $security_deposit_enabled = \App\Models\BusinessSetting::where('key', 'security_deposit_enabled')->first()?->value == '1';
                                    $security_deposit_amount = \App\Models\BusinessSetting::where('key', 'security_deposit_amount')->first()?->value ?? 0;
                                @endphp
                                @if($security_deposit_enabled)
                                    @if($dm->security_deposit_status == 'paid')
                                        <span class="badge badge-soft-success">
                                            <i class="tio-checkmark-circle"></i> {{ translate('messages.paid') }}
                                        </span>
                                        <div class="text-muted small">
                                            {{ \App\CentralLogics\Helpers::currency_symbol() }}{{ number_format($dm->security_deposit_amount ?? $security_deposit_amount, 2) }}
                                        </div>
                                        @if($dm->security_deposit_paid_at)
                                            <div class="text-muted small">
                                                {{ \App\CentralLogics\Helpers::time_date_format($dm->security_deposit_paid_at) }}
                                            </div>
                                        @endif
                                    @elseif($dm->security_deposit_status == 'refunded')
                                        <span class="badge badge-soft-info">
                                            <i class="tio-refresh"></i> {{ translate('messages.refunded') }}
                                        </span>
                                    @else
                                        <span class="badge badge-soft-warning">
                                            <i class="tio-time"></i> {{ translate('messages.unpaid') }}
                                        </span>
                                        <div class="text-muted small">
                                            {{ \App\CentralLogics\Helpers::currency_symbol() }}{{ number_format($security_deposit_amount, 2) }}
                                        </div>
                                    @endif
                                @else
                                    <span class="badge badge-soft-secondary">
                                        {{ translate('messages.not_required') }}
                                    </span>
                                @endif
                            </td>
                            <td>
