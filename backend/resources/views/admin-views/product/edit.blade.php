                ...
                <div class="col-md-6">
                    <div class="card h-100">
                        <div class="card-body">
                            ...
                            @endif
                            @if ($language)
                            <div class="col-sm-6 col-lg-3" id="barcode_input">
                                <div class="form-group mb-0">
                                    <label class="input-label" for="barcode">{{ translate('messages.Barcode') }}
                                        <span class="input-label-secondary" data-toggle="tooltip" data-placement="right"
                                            data-original-title="{{ translate('messages.Enter_product_barcode_number') }}">
                                            <i class="tio-info-outined"></i>
                                        </span>
                                    </label>
                                    <input type="text" 
...
@if(!empty($images))
    @foreach($images as $key => $photo)
        ...
        <div id="product_images_{{ $key }}" class="spartan_item_wrapper min-w-176px max-w-176px position-relative">
            ...
            <a href="javascript:void(0)"
               ...
               data-src="{{ \App\CentralLogics\Helpers::get_full_url('product', $photo['img'] ?? '', $photo['storage']) }}"
    ...
    @endforeach
                                    @else
                                        <div class="text-center text-muted">
                                            {{ translate('messages.no_images_available') }}
                                            <!-- Or show a placeholder image -->
                                            <!-- <img src="{{ asset('public/assets/admin/img/upload-img.png') }}" class="img--square" alt="No image"> -->
                                        </div>
                                    @endif
                                </div>
                            </div>
...
<div class="col-sm-6 col-lg-3" id="unit_input">
    <div class="form-group mb-0">
        <label class="input-label text-capitalize d-flex align-items-center justify-content-between" for="unit">
            {{ translate('messages.unit') }}
            <button type="button" class="btn btn-sm btn-light border" id="refreshUnitBtn" title="Refresh Units">
                <i class="tio-refresh"></i>
            </button>
        </label>
        <select name="unit" id="unitSelect" class="form-control js-select2-custom">
            ...
            @endforeach
                                ...
                                <div class="col-sm-6 col-lg-3" id="maximum_cart_quantity">
                                 ...
                                 <div class="col-sm-6 col-lg-4" id="rack">
                                    <div class="form-group mb-0">
                                        <label class="input-label"
                                            for="rack">{{ translate('messages.Rack') }}
                                        </label>
                                        <input type="text" placeholder="{{ translate('messages.Ex:R1,1,R-1') }}" class="form-control" name="rack" min="0" value="{{ $product->rack }}" id="rack">
                                    </div>
                                </div>
                                 <div class="col-sm-6 col-lg-4" id="row">
                                    <div class="form-group mb-0">
                                        <label class="input-label"
                                            for="rowy">{{ translate('messages.row') }}
                                            
                                        </label>
                                        <input type="number" placeholder="{{ translate('messages.Ex:1 , 2') }}" class="form-control" name="row" min="0" value="{{ $product->row }}" id="row">
                                    </div>
                                </div>
...
<script>
    ...
    $(document).on('click', '.edit-image-btn', function(e) {
        ...
        openImageEditor(imgSrc, {
            sourceType: 'existing',
            imageName: imageName,
            saveUrl: '{{ route("admin.item.save-edited-image") }}',
            removeBgUrl: '{{ route("admin.item.remove-bg") }}',
...
// Simple refresh button for Units
$('#refreshUnitBtn').on('click', function() {
    $.ajax({
        url: "{{ url('/admin/item/get-units-refresh') }}",
        type: "GET",
        success: function(html) {
            $('#unitSelect').html(html);
            toastr.success('Units refreshed!');
        },
        error: function() {
            toastr.error('Failed to refresh units.');
        }
    });
});
