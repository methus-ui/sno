    ...
    <div class="content container-fluid">
        ...
        <form action="{{ route('admin.business-settings.zone.module-update', $zone->id) }}" method="post" id="zone_form" class="shadow--card">
            ...
            <div class="row g-2">
                ...
                <!-- Time-Based Delivery Fee Schedules Section -->
                <div class="row mt-3">
                    <div class="col-12">
                        <div class="card border">
                            <div class="card-body py-3">
                                <div class="d-flex justify-content-between align-items-center">
                                    <div>
                                        <h6 class="mb-1">
                                            <i class="tio-time text-primary"></i>
                                            {{ translate('Time-Based Delivery Fee Schedules') }}
                                        </h6>
                                        <p class="mb-0 small text-muted">
                                            {{ translate('Configure automated delivery fee adjustments based on day and time') }}
                                        </p>
                                        @php($scheduleCount = $zone->activeDeliveryFeeSchedules()->count())
                                        @if($scheduleCount > 0)
                                            <span class="badge badge-soft-success mt-1">
                                                {{ $scheduleCount }} {{ translate('active schedule(s)') }}
                                            </span>
                                        @endif
                                    </div>
                                    <a href="{{ route('admin.business-settings.zone.delivery-fee-schedule.index', $zone->id) }}"
                                       class="btn btn-primary btn-sm">
                                        <i class="tio-settings"></i>
                                        {{ translate('Manage Schedules') }}
                                    </a>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="card border">
                            <div class="card-body py-3">
                                <div class="d-flex justify-content-between align-items-center">
                                    <div>
                                        <h6 class="mb-1">
                                            <i class="tio-cloud text-warning"></i>
                                            {{ translate('Weather-Based Surge') }}
                                        </h6>
                                        <p class="mb-0 small text-muted">
                                            {{ translate('Auto-increase delivery fee during bad weather') }}
                                        </p>
                                    </div>
                                    <div class="d-flex align-items-center">
                                        <label class="toggle-switch toggle-switch-sm mr-2 mb-0">
                                            <input type="checkbox" class="toggle-switch-input" name="weather_surge_enabled" value="1"
                                                {{ $zone->weather_surge_enabled ? 'checked' : '' }}>
                                            <span class="toggle-switch-label"><div class="toggle-switch-indicator"></div></span>
                                        </label>
                                        <a href="{{ route('admin.business-settings.zone.weather-surge.index', $zone->id) }}"
                        ...
                        <div class="card border">
                            <div class="card-body py-3">
                                <div class="d-flex justify-content-between align-items-center">
                                    <div>
                                        <h6 class="mb-1">
                                            <i class="tio-trending-up text-danger"></i>
                                            {{ translate('Demand-Based Surge') }}
                                        </h6>
                                        <p class="mb-0 small text-muted">
                                            {{ translate('Auto-increase fee when demand is high') }}
                                        </p>
                                    </div>
                                    <div class="d-flex align-items-center">
                                        <label class="toggle-switch toggle-switch-sm mr-2 mb-0">
                                            <input type="checkbox" class="toggle-switch-input" name="demand_surge_enabled" value="1"
                                                {{ $zone->demand_surge_enabled ? 'checked' : '' }}>
                                            <span class="toggle-switch-label"><div class="toggle-switch-indicator"></div></span>
                                        </label>
                                        <a href="{{ route('admin.business-settings.zone.demand-surge.index', $zone->id) }}"
