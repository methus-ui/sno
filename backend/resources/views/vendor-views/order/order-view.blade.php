        ...
        <div class="page-header">
            <div class="row align-items-center">
                ...
                <div class="col-sm-auto">
                    ...
                    <a href="{{ route('vendor.order.view-v2', $order->id) }}" class="btn btn-sm btn-soft-success mr-2" data-toggle="tooltip" data-placement="top" title="{{ translate('messages.view_bargaining_details') }}">
            ...
            <div class="col-lg-8 mb-3 mb-lg-0">
                ...
                <div class="card mb-3 mb-lg-5">
                    ...
                    <div class="card-body px-0">
                                ...
                                <tbody>
                                    @foreach ($order->details as $key => $detail)
                                        @if (isset($detail->item_id))
                                            ...
                                            <!-- Media -->
                                            <tr data-order-detail-id="{{ $detail->id }}">
                                                <td>
                                                    <div>
                                                        {{ $key + 1 }}
                                                    </div>
                                                        ...
                                                        <div class="media-body">
                                                            <div>
                                                                ...
                                                                @if($detail->is_picked_up)
                                                                    <span class="badge badge-info ml-1" style="font-size: 11px;" title="Picked up at {{ $detail->picked_up_at ? $detail->picked_up_at->format('h:i A') : '' }}">
                                                                        <i class="tio-shopping-basket-outlined"></i> {{ translate('messages.picked_up') }}
                                                                    </span>
                                                                @endif
                                                                @if($detail->mrp_update_status)
                                                                    <span class="badge badge-{{ $detail->mrp_update_status == 'pending' ? 'info' : ($detail->mrp_update_status == 'approved' ? 'success' : 'danger') }} ml-1">
                                        ...
                                        @elseif(isset($detail->item_campaign_id))
                                            ...
                                            <tr>
                                                ...
                                                {{-- Availability Status Column for Campaign Items --}}
                                                <td class="text-center align-middle">
                                                    @if($detail->is_unavailable)
                                                        <span class="badge badge-soft-danger" style="font-size: 13px; padding: 8px 12px;">
                                                            <i class="tio-clear-circle-outlined"></i>
                        ...
                        {{-- ✅ ORDER EDIT HISTORY SECTION --}}
                        @if(isset($transaction) && $transaction && $transaction->is_edited && !empty($editHistory))
                        <div class="row justify-content-md-end mx-0 mt-3">
                            <div class="col-md-9 col-lg-8">
                                {{-- Transaction Summary Card --}}
                                <div class="card border-0 shadow-sm mb-3" style="border-left: 4px solid #667eea !important;">
                                    <div class="card-header bg-gradient-primary text-white py-2">
                                        <h6 class="mb-0" style="font-size: 13px;">
                                            <i class="tio-money"></i> {{ translate('messages.transaction_audit') }}
                                            <span class="badge badge-light text-primary ml-2">{{ $transaction->edit_count }} {{ $transaction->edit_count == 1 ? translate('messages.edit') : translate('messages.edits') }}</span>
                                        </h6>
                                    </div>
                                    <div class="card-body p-3">
                                        <table class="table table-sm table-borderless mb-0" style="font-size:11px;">
                                            <tr>
                                                <td class="text-muted py-1">{{ translate('messages.original_amount') }}</td>
                                                <td class="text-right py-1 font-weight-bold">
                                                    {{ \App\CentralLogics\Helpers::format_currency($transaction->original_order_amount) }}
                                                </td>
                                            </tr>
                                            <tr>
                                                <td class="text-muted py-1">{{ translate('messages.current_amount') }}</td>
                                                <td class="text-right py-1 font-weight-bold">
                                                    {{ \App\CentralLogics\Helpers::format_currency($transaction->order_amount) }}
                                                </td>
                                            </tr>
                                            <tr>
                                                <td class="text-muted py-1">{{ translate('messages.total_adjustment') }}</td>
                                                <td class="text-right py-1">
                                                    <span class="badge badge-{{ $transaction->total_adjustment >= 0 ? 'success' : 'danger' }} font-weight-bold">
                                                        {{ $transaction->total_adjustment >= 0 ? '+' : '' }}{{ \App\CentralLogics\Helpers::format_currency($transaction->total_adjustment) }}
                                                    </span>
                                                </td>
                                            </tr>
                                            @if($transaction->original_store_amount)
                                            <tr>
                                                <td colspan="2"><hr class="my-1"></td>
                                            </tr>
                                            <tr>
                                                <td class="text-muted py-1">{{ translate('messages.original_store_amount') }}</td>
                                                <td class="text-right py-1">{{ \App\CentralLogics\Helpers::format_currency($transaction->original_store_amount) }}</td>
                                            </tr>
                                            <tr>
                                                <td class="text-muted py-1">{{ translate('messages.current_store_amount') }}</td>
                                            ...
                                            <tr>
                                                <td class="text-muted py-1">{{ translate('messages.last_edited') }}</td>
                                                <td class="text-right py-1">
                                                    {{ \Carbon\Carbon::parse($transaction->last_edited_at)->format('d M Y, h:i A') }}
                                                </td>
                                            </tr>
                                        </table>
                                    </div>
                                        ...
                                        <div class="timeline-container p-3" style="max-height: 600px; overflow-y: auto;">
                                            ...
                                            <div class="timeline-item mb-4 pb-3" style="border-left: 3px solid {{ $edit['adjustment'] >= 0 ? '#28a745' : '#dc3545' }}; padding-left: 20px; position: relative; background: #f8f9fa; border-radius: 0 8px 8px 0; padding: 15px 15px 15px 20px;">
                                                ...
                                                <div style="position: absolute; left: -12px; top: -10px;">
                                                    <span class="badge badge-{{ $edit['adjustment'] >= 0 ? 'success' : 'danger' }}" style="border-radius: 50%; width: 24px; height: 24px; display: flex; align-items: center; justify-content: center; font-size: 10px; font-weight: bold;">
                                                        #{{ count($editHistory) - $index }}
                                                    </span>
                                                </div>
                                                ...
                                                <div class="d-flex justify-content-between align-items-start mb-3">
                                                    <div>
                                                        ...
                                                        <div class="text-muted mt-1" style="font-size: 11px;">
                                                            ...
                                                            <span class="ml-2">
                                                                <i class="tio-time"></i> {{ \Carbon\Carbon::parse($edit['edited_at'])->diffForHumans() }}
                                                            </span>
                                                        </div>
                                                    </div>
                                                ...
                                                <div class="card mb-3 border-0 shadow-sm">
                                                    <div class="card-header bg-white py-2">
                                                        <strong style="font-size: 12px;">
                                                            <i class="tio-money"></i> {{ translate('messages.order_amount_change') }}
                                                        </strong>
                                                    </div>
                                                ...
                                                {{-- Detailed Changes --}}
                                                @if(isset($edit['changes']) && !empty($edit['changes']))
                                                <div class="card border-0 shadow-sm">
                                                    <div class="card-header bg-white py-2">
                                                        <strong style="font-size: 12px;">
                                                            <i class="tio-edit"></i> {{ translate('messages.detailed_changes') }}
                                                        </strong>
                                                    </div>
                                                    <div class="card-body p-2">
                                                        <table class="table table-sm table-bordered mb-0" style="font-size: 11px;">
                                                            @foreach($edit['changes'] as $key => $value)
                                                            <tr>
                                                                ...
                                                                <td>
                                                                    ...
                                                                    @elseif(is_array($value))
                                                                        <pre style="font-size: 10px; margin: 0; max-height: 150px; overflow-y: auto;">{{ json_encode($value, JSON_PRETTY_PRINT) }}</pre>
                        ...
                        <div class="row justify-content-md-end mx-0 mt-3">
                            <div class="col-md-9 col-lg-8">
                                <div class="alert alert-info" style="font-size: 12px;">
                                    <i class="tio-checkmark-circle"></i> {{ translate('messages.order_not_edited') }} - {{ translate('messages.original_transaction_intact') }}
                                </div>
                            </div>
                        </div>
                        @endif
                ...
                @if ($order->is_billed || ($bill_data != null && count($bill_data) > 0))
                    <div class="card mb-2 mt-2">
                        <div class="card-header border-0 pb-0">
                            <h4 class="m-0">{{ translate('messages.bill_image') }}
                                @if($order->is_billed)
                                    <span class="badge badge-success ml-2">{{ translate('messages.billed') }}</span>
                                @endif
                            </h4>
                            @if($order->billed_at)
                                <small class="text-muted">{{ $order->billed_at->format('d M Y, h:i A') }}</small>
                            @endif
                        </div>
                        <div class="card-body pt-2">
                            @if ($bill_data)
                                ...
                                <div class="row g-3">
                                    @foreach ($bill_data as $bkey => $bimg)
                                        @php($bimg = is_array($bimg)?$bimg:['img'=>$bimg,'storage'=>'public'])
                                        <div class="col-4">
                                            <img class="img__aspect-1 rounded border w-100 onerror-image" data-toggle="modal"
                                                 data-target="#billimagemodal{{ $bkey }}"
                                                 data-onerror-image="{{ asset('public/assets/admin/img/160x160/img2.jpg') }}"
                                                 src="{{ \App\CentralLogics\Helpers::get_full_url('order',$bimg['img'],$bimg['storage']) }}">
                                        </div>
                                        <div class="modal fade" id="billimagemodal{{ $bkey }}" tabindex="-1"
                                             role="dialog" aria-labelledby="bill_image_{{ $bkey }}"
                                             aria-hidden="true">
                                            <div class="modal-dialog">
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                        <h4 class="modal-title" id="bill_image_{{ $bkey }}">
                                                            {{ translate('messages.bill_image') }}</h4>
                                                    ...
                                                    @php($bstorage = $bimg['storage'] ?? 'public')
                                                    @php($bfile = $bstorage == 's3'?base64_encode('order/' . $bimg['img']):base64_encode('public/order/' . $bimg['img']))
                                                    <div class="modal-footer">
                                                        <a class="btn btn-primary"
                                                           href="{{ route('admin.file-manager.download', [$bfile,$bstorage]) }}">
                                ...
                                {{-- Bill Scan Results (Read-only for vendors) --}}
