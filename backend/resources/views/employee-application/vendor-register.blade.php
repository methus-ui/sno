...
<head>
    ...
    <title>{{ translate('messages.vendor_employee_registration') }} | {{ $business_name }}</title>
    <meta name="description" content="{{ translate('messages.vendor_employee_application') }} - {{ translate('messages.order_fulfillment_specialist') }}">

    <!-- Open Graph / Social Media Meta Tags -->
    <meta property="og:type" content="website">
    <meta property="og:title" content="{{ translate('messages.vendor_employee_registration') }} | {{ $business_name }}">
    <meta property="og:description" content="{{ translate('messages.warehouse_picker') }} - {{ translate('messages.join_kashmir_success_story') }}">
    <meta property="og:site_name" content="{{ $business_name }}">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:title" content="{{ translate('messages.vendor_employee_registration') }} | {{ $business_name }}">
                ...
                <div class="card shadow">
                    <div class="card-body p-5">
                        <div class="text-center mb-4">
                            <h2 class="h3" style="color: #D82E5E;">{{ translate('messages.vendor_employee_application') }}</h2>
                            <p class="text-muted">{{ translate('messages.fill_form_to_apply') }}</p>
                        </div>

                        <form method="POST" action="{{ route('employee.register.vendor.submit') }}" enctype="multipart/form-data">
                            @csrf
                            @if($invitation)
                                    ...
                                    <select name="store_id" class="form-control" required>
                                        ...
                                        @foreach($stores as $store)
                                            <option value="{{ $store->id }}"
                                                    {{ old('store_id', $invitationData['store_id'] ?? '') == $store->id ? 'selected' : '' }}>
                                                {{ $store->name }}
                                            </option>
                                        @endforeach
                                    </select>
