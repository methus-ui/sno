<div class="row">
    <div class="col-lg-12 text-center">
        <h1>{{ translate($data['status']) }} {{ translate('messages.order_list') }}</h1>
    </div>
            ...
            <thead>
                <tr>
                    ...
                    <th>
                        ...
                        @if ($data['search'])
                            <br>{{ translate('search_bar_content') }} : {{ $data['search'] }}
                        @endif
                        @if ($data['zones'])
                            <br>{{ translate('zones') }} : {{ $data['zones'] }}
                        @endif
                        @if ($data['stores'])
                            <br>{{ translate('stores') }} : {{ $data['stores'] }}
                        @endif
                        @if ($data['type'])
                            <br>{{ translate('order_type') }} : {{ translate($data['type']) }}
                        @endif
                        @if ($data['from'])
                ...
                <tr>
                    ...
                    @endif
                    <th>{{ translate('messages.delivery_date') }}</th>
                    <th>{{ translate('messages.delivery_boy') }}</th>
                    <th>{{ translate('messages.assigned_to') }}</th> {{-- ✅ Added --}}
                    ...
                    <tr>
                        ...
                        <td>
                            @if ($order->delivered && $order->delivered_at)
                                {{ \App\CentralLogics\Helpers::time_date_format($order->delivered_at) }}
                            @else
                                {{ translate('messages.not_delivered_yet') }}
                            @endif
                        </td>
                        ...
                        {{-- Item Price --}}
                        <td>
                            {{ number_format($order['order_amount'] - $order['dm_tips'] - $order['total_tax_amount'] - $order['delivery_charge'] + $order['coupon_discount_amount'] + $order['store_discount_amount'], 2) }}
                        </td>

                        {{-- Item Discount --}}
                        <td>{{ number_format($order->details->sum('discount_on_item'), 2) }}</td>

                        {{-- Coupon Discount --}}
                        <td>{{ number_format($order['coupon_discount_amount'], 2) }}</td>

                        {{-- Discounted Amount --}}
                        <td>{{ number_format($order['coupon_discount_amount'] + $order['store_discount_amount'] + $order['ref_bonus_amount'], 2) }}</td>
