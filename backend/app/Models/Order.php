...
class Order extends Model
{
    ...
    protected $casts = [
        ...
        'is_billed' => 'boolean',
        'billed_at' => 'datetime',
        'bill_scan_result' => 'array',
        'dm_was_at_location_on_cancel' => 'boolean',
        'dm_distance_on_cancel' => 'float',
        'dm_cancellation_compensation' => 'float',
        'is_bargaining_order' => 'boolean',
    ];

    protected $appends = ['module_type','order_attachment_full_url','order_proof_full_url','bill_image_full_url'];
    ...
    public function getBillImageFullUrlAttribute(){
        $images = [];
        $value = is_array($this->bill_image)
            ? $this->bill_image
            : ($this->bill_image && is_string($this->bill_image) && $this->isValidJson($this->bill_image)
                ? json_decode($this->bill_image, true)
                : []);
        if ($value){
            foreach ($value as $item){
                $item = is_array($item)?$item:(is_object($item) && get_class($item) == 'stdClass' ? json_decode(json_encode($item), true):['img' => $item, 'storage' => 'public']);
    ...
    /**
     * Get delivery man's last known location.
     * This is an accessor (not a relationship) because it goes through delivery_man.
     */
    public function getDmLastLocationAttribute()
    {
        if (!$this->delivery_man) {
            return null;
        }

        // Use the delivery_man's last_location relationship
        $location = $this->delivery_man->last_location;
    ...
    {
        return $this->hasOne(DeliveryTrackingStat::class, 'order_id');
    }
    ...
    public function scopeNotDigitalOrder($query)
    {
        return $query->where(function ($q){
            $q->whereNotIn('payment_method', ['digital_payment','offline_payment'])
              ->orWhere(function($qq) {
                  $qq->whereIn('payment_method', ['digital_payment','offline_payment'])
