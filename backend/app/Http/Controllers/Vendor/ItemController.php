...
class ItemController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->has('image') && $food->image) {
            ...
            if ($request->has('process_images') && $request->process_images == '1') {
                Helpers::processProductImage('product/', $food->image);
            }
        }
    ...
    public function update(Request $request, $id)
    {
        ...
        // Prevent updating products from product gallery (should use replicate instead)
        if($request->product_gellary == 1) {
            return response()->json([
                'errors'=>[
                    ['code'=>'invalid_action', 'message'=>translate('messages.cannot_update_gallery_product_use_replicate')]
                ]
            ], 400);
        }
        ...
        if (Helpers::get_mail_status('product_approval') && ((data_get($product_approval_datas,'Update_anything_in_product_details',null) == 1) || (data_get($product_approval_datas,'Update_product_price',null) == 1 && $old_price !=  $request->price) || ( data_get($product_approval_datas,'Update_product_variation',null) == 1 &&  $variation_changed)) )  {
            ...
            if ($request->has('image') && $request->file('image')) {
                Helpers::processProductImage('product/', $p->image);
            }
            $images = is_array($p['images']) ? $p['images'] : (is_string($p['images']) ? json_decode($p['images'], true) ?? [] : []);

            foreach($images as $key=> $value){
                if( in_array( is_array($value) ?   $value['img'] : $value ,explode(",", $request->removedImageKeys))) {
                    $value = is_array($value)?$value:['img' => $value, 'storage' => 'public'];
                    Helpers::check_and_delete('product/' , $value['img'], $p->id);
                    unset($images[$key]);
                }
                }
    ...
    public function search(Request $request){
        ...
        if(isset($request->product_gallery) && $request->product_gallery==1 && $settings_access == 1){
                ...
                $items= $items->withoutGlobalScope(StoreScope::class)
                    // Order by relevance: exact barcode > exact name > starts with > partial match
                    ->orderByRaw("
                        CASE
                            WHEN barcode = ? THEN 1
                            WHEN name = ? THEN 2
                            ...
                            WHEN store_id = ? THEN 5
                            ELSE 6
                        END
                    ", [$keyword, $keyword, $keyword.'%', $keyword.'%', Helpers::get_store_id()])
                    ->limit(100)->get(); // Increased from 12 to 100 for search results
...
//                    DB::table('items')->upsert($chunk_item,['id','module_id'],['name','description','image','images','category_id','category_ids','unit_id','stock','price','discount','discount_type','available_time_starts','available_time_ends','variations','food_variations','add_ons','attributes','store_id','status','veg','recommended', 'updated_at','choice_options']);
    ...
    public function store_temp_data($data, $request,$tag_ids, $nutrition_ids, $allergy_ids , $generic_ids, $update =null)
    {
        ...
        $temp_item->barcode = $request->barcode;
    ...
    public function product_gallery(Request $request){
        ...
        ->type($type)
        ->orderByRaw('CASE WHEN store_id = ? THEN 0 ELSE 1 END', [Helpers::get_store_id()])
        ->inRandomOrder()
        ->module(Helpers::get_store_data()->module_id)
        ->paginate(32); // Changed from limit(12)->get() to paginate(32)
    ...
    public function searchByBarcode(Request $request)
    {
        try {
            ...
            if (empty($barcode)) {
                return response()->json(['found' => false, 'message' => 'Barcode is required']);
            }

            $item = \App\Models\Item::where('barcode', $barcode)
            ...
            return response()->json([
                'found' => false,
                'message' => "Product with barcode '{$barcode}' not found in your store"
            ]);

        } catch (\Exception $e) {
            return response()->json(['found' => false, 'message' => 'Server error: ' . $e->getMessage()], 500);
        }
    }
