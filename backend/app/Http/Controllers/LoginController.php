...
// dd($request->all());
        ...
        if ($request->role == 'admin_employee') {
            ...
            $adminEmployee = Admin::where('email', $request->email)->where('role_id', '!=', 1)->first();
            if ($adminEmployee) {
                // Check if application is pending (status = NULL)
                if (is_null($adminEmployee->status)) {
                    return redirect()->back()->withInput($request->only('email', 'remember'))
                        ->withErrors([translate('messages.your_application_is_pending_approval')]);
                }
                // Check if application was denied (status = 0)
                if ($adminEmployee->status === 0) {
                    return redirect()->back()->withInput($request->only('email', 'remember'))
                        ->withErrors([translate('messages.your_application_has_been_denied')]);
                }
                // Only status = 1 (approved) can proceed to login
            }
            ...
            if ($employee) {
                ...
                // Check if application is pending (application_status = NULL)
                if (is_null($employee->application_status)) {
                    return redirect()->back()->withInput($request->only('email', 'remember'))
                        ->withErrors([translate('messages.your_application_is_pending_approval')]);
                }
                // Check if application was denied (application_status = 0)
                if ($employee->application_status === 0) {
                    return redirect()->back()->withInput($request->only('email', 'remember'))
                        ->withErrors([translate('messages.your_application_has_been_denied')]);
                }
