...
class SearchRoutingController extends Controller
{
    public function index(Request $request)
    {
        ...
        // Early return for empty or very short searches
        if (empty($searchKeyword) || strlen($searchKeyword) < 1) {
        ...
        //1st layer - Cache JSON routes for 24 hours
        $formattedRoutes = [];
        $cacheKey = 'admin_formatted_routes_' . md5($currentModuleType . '_' . addon_published_status('Rental'));
        ...
        // //2nd layer - Cache admin routes collection for 24 hours
        ...
        if (is_numeric($searchKeyword) && $searchKeyword > 0) {
            //store
            $store = Store::select('id', 'name', 'module_id', 'vendor_id')
                ->with('vendor:id,status')
            ...
            if ($currentModuleType !== 'rental') {
                ...
                //order
                $order = Order::select('id', 'order_status', 'module_id')
                    ->when(is_numeric($currentModuleId), function ($query) use ($currentModuleId) {
                        return $query->where('module_id', $currentModuleId);
                    })->find($searchKeyword);
                if ($order) {
                ...
                //multiple orders with customer id (limit to 10 results for performance)
                $orders = Order::select('id', 'order_status', 'module_id', 'user_id', 'store_id')
                    ->with(['customer:id,f_name,l_name', 'store:id,name'])
                    ->when(is_numeric($currentModuleId), function ($query) use ($currentModuleId) {
                        return $query->where('module_id', $currentModuleId);
                    })
                    ->where(function ($query) use ($searchKeyword) {
                        $query->whereHas('customer', function ($q) use ($searchKeyword) {
                            $q->where('id', $searchKeyword);
                        })
                        ->orWhereHas('store', function ($q) use ($searchKeyword) {
                            $q->where('id', $searchKeyword);
                        });
                    })
                    ->limit(10)
                ...
                //category
                $category = Category::select('id', 'name', 'module_id', 'parent_id')
                    ->when(is_numeric($currentModuleId), function ($query) use ($currentModuleId) {
                        return $query->where('module_id', $currentModuleId);
                    })->find($searchKeyword);
                ...
                //delivery man
                $deliveryMan = DeliveryMan::select('id', 'f_name', 'l_name', 'status', 'application_status')
                    ->find($searchKeyword);
                if ($deliveryMan) {
            ...
            //customer
            $customer = User::select('id', 'f_name', 'l_name', 'email', 'phone')
                ->find($searchKeyword);
            if ($customer) {
        ...
        $result = $result->unique('fullRoute')->values();

        // Limit results to top 50 for performance
        $result = $result->take(50)->all();
