   ...
   public function getUpdateView(string|int $id): View|RedirectResponse
    {
        ...
        if (!$zone || !isset($zone->coordinates)) {
            Toastr::error(translate('messages.zone_not_found_or_invalid_coordinates'));
            return back();
        }

        // Get all zones with their coordinates and centers
        $allZones = $this->zoneRepo->getAll();
        $zonesData = [];
        ...
        foreach ($allZones as $z) {
            try {
                $zoneWithCoords = $this->zoneRepo->getWithCoordinateWhere(['id' => $z->id]);
                
                if (!$zoneWithCoords || !isset($zoneWithCoords->coordinates)) {
                    continue;
                }

                // Parse coordinates from WKT format
                $coordinates = $this->parseWktPolygon($zoneWithCoords->coordinates);
                
                if (empty($coordinates)) {
                    continue;
                }

                // Parse center from WKT POINT format
                $center = $this->parseWktPoint($zoneWithCoords->center);
                ...
                $zonesData[] = [
                    ...
                    'center' => $center,
                    'color' => $z->id == $id ? '#050df2' : $colors[$colorIndex++ % count($colors)]
                ];
            } catch (\Exception $e) {
                continue;
            }
        ...
        try {
            $area = $this->parseWktPolygon($zone->coordinates);
            
            if (empty($area)) {
                throw new \Exception('Invalid zone coordinates');
            }
        } catch (\Exception $e) {
            Toastr::error(translate('messages.invalid_zone_coordinates'));
            return back();
        }

        $language = getWebConfig('language');
        $defaultLang = str_replace('', '-', app()->getLocale());

        return view(ZoneViewPath::UPDATE[VIEW], compact(['zone', 'area', 'language', 'defaultLang', 'zonesData']));
    }

    /**
     * Parse WKT polygon string to coordinates array
     */
    protected function parseWktPolygon($wkt): array
    {
        if (is_object($wkt) && method_exists($wkt, 'toWkt')) {
            $wkt = $wkt->toWkt();
        }
        ...
        if (preg_match('/POLYGON\(\((.*?)\)\)/', $wkt, $matches)) {
            ...
            foreach ($points as $point) {
                ...
                if (count($coords) >= 2) {
                    $coordinates[] = [
                        floatval($coords[0]), // lng
                        floatval($coords[1])  // lat
                    ];
                }
            }
    ...
    /**
     * Parse WKT point string to center coordinates
     */
    protected function parseWktPoint($wkt): array
    {
        if (is_object($wkt) && method_exists($wkt, 'toWkt')) {
            $wkt = $wkt->toWkt();
        }
