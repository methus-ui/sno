...
class SystemController extends Controller
{
    ...
    public function orders_live_status(Request $request)
    {
        ...
        $orders = Order::whereIn('id', $orderIds)
            ->with(['delivery_man:id,f_name,l_name,phone,image', 'assignedAdmin:id,f_name,l_name'])
            ->select('id', 'order_status', 'payment_status', 'is_billed', 'billed_at', 'delivery_man_id', 'assigned_to', 'order_amount')
            ->get();
        ...
        foreach ($orders as $o) {
            ...
            $result[$o->id] = [
                ...
                'order_amount' => \App\CentralLogics\Helpers::format_currency($o->order_amount),
                'is_billed' => (bool)$o->is_billed,
                'billed_at' => $o->billed_at ? \Carbon\Carbon::parse($o->billed_at)->format('d M Y, h:i A') : null,
                'delivery_man' => $dm ? [
                    'id' => $dm->id,
                ...
                'assigned_admin' => $o->assignedAdmin ? $o->assignedAdmin->f_name . ' ' . $o->assignedAdmin->l_name : null,
            ];
        }
    ...
    public function check_new_bills()
    {
        $lastChecked = session('last_bill_check', now()->subSeconds(15));
        $newBills = Order::where('is_billed', true)
            ->where('billed_at', '>', $lastChecked)
            ->with('store:id,name')
            ->select('id', 'store_id', 'billed_at')
            ->latest('billed_at')
            ->limit(5)
            ->get();

        session(['last_bill_check' => now()]);

        return response()->json([
            'count' => $newBills->count(),
            'bills' => $newBills->map(function ($o) {
                return [
                    'order_id' => $o->id,
                    'store_name' => $o->store->name ?? 'N/A',
