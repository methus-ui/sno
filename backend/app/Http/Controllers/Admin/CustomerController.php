...
public function view(Request $request, $id)
{
    ...
    if (isset($customer)) {
        $total_order_amount = Order::selectRaw('sum(order_amount) as total_order_amount')
            ->latest()
            ->where(['user_id' => $id])
        ...
        // ============ UPDATED: Get Customer's Active Cart ============
        try {
            // Get cart items for this customer
            $customerCart = \DB::table('carts')
                ->where('user_id', $id)
                ->orderBy('created_at', 'DESC')
                ->get();
            
            // Debug log
            \Log::info('Customer Cart Query', [
                'user_id' => $id,
                'cart_count' => $customerCart->count()
            ]);
            
            // Get all item IDs from cart
            if ($customerCart->count() > 0) {
                $itemIds = $customerCart->pluck('item_id')->unique();
            ...
            \Log::error('Customer cart error: ' . $e->getMessage());
            $customerCart = collect();
            $cartItems = collect();
        }
        // ============ END: Customer Cart ============
