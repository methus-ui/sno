...
class BusinessSettingsController extends Controller
{
    ...
    public function update_dm(Request $request)
    {
        ...
        Helpers::businessUpdateOrInsert(['key' => 'dm_cancellation_compensation_enabled'], [
            'value' => $request['dm_cancellation_compensation_enabled'] ?? 0
        ]);

        Helpers::businessUpdateOrInsert(['key' => 'dm_cancellation_compensation_percentage'], [
            'value' => $request['dm_cancellation_compensation_percentage'] ?? 50
        ]);

        // Convert meters to kilometers
        $proximityMeters = $request['dm_cancellation_compensation_proximity'] ?? 200;
        Helpers::businessUpdateOrInsert(['key' => 'dm_cancellation_compensation_proximity'], [
            'value' => $proximityMeters / 1000
        ]);

        // Security Deposit Settings
        Helpers::businessUpdateOrInsert(['key' => 'security_deposit_enabled'], [
            'value' => $request['security_deposit_enabled'] ?? 0
        ]);

        Helpers::businessUpdateOrInsert(['key' => 'security_deposit_amount'], [
            'value' => $request['security_deposit_amount'] ?? 500
        ]);
    ...
    public function update_order(Request $request)
    {
        ...
        Helpers::businessUpdateOrInsert(['key' => 'delivery_proximity_enforcement'], [
            'value' => $request['delivery_proximity_enforcement'] ?? 0
        ]);

        // Convert meters to kilometers for storage
        $proximityMeters = $request['delivery_proximity_radius'] ?? 200;
        Helpers::businessUpdateOrInsert(['key' => 'delivery_proximity_radius'], [
            'value' => $proximityMeters / 1000
        ]);
    ...
    public function business_setup(Request $request)
    {
        ...
        // Save tiered additional charge settings
        Helpers::businessUpdateOrInsert(['key' => 'additional_charge_tiered_enabled'], [
            'value' => $request['additional_charge_tiered_enabled'] ? 1 : 0
        ]);

        // Process and save tier configuration
        if ($request->has('tier_min') && is_array($request->tier_min)) {
            $tiers = [];
            foreach ($request->tier_min as $index => $min) {
                $max = $request->tier_max[$index] ?? null;
                $charge = $request->tier_charge[$index] ?? 0;

                // Convert empty max to null (no upper limit)
                if ($max === '' || $max === null) {
                    $max = null;
                } else {
                    $max = (float)$max;
            ...
            // Sort tiers by min value (lowest to highest)
            usort($tiers, function($a, $b) {
                return $a['min'] <=> $b['min'];
            });

            Helpers::businessUpdateOrInsert(['key' => 'additional_charge_tiers'], [
...
//        file_put_contents(base_path('.env'), implode(PHP_EOL, $lines) . PHP_EOL);
    ...
    public function systemCheck()
    {
        ...
        try {
            ...
            $maxConn = DB::selectOne('SHOW VARIABLES LIKE "max_connections"');
            $usedConn = DB::selectOne('SHOW STATUS LIKE "Threads_connected"');
            $checks['mysql'] = [
                'status' => 'ok',
                'version' => $mysqlVersion,
                'uptime_seconds' => $uptime->Value ?? 0,
                'max_connections' => $maxConn->Value ?? 'N/A',
                'used_connections' => $usedConn->Value ?? 'N/A',
            ];
        } catch (\Exception $e) {
        ...
        $checks['disk'] = [
            'status' => 'ok',
            'storage' => [
                'free' => round(disk_free_space($storagePath) / 1073741824, 2),
                'total' => round(disk_total_space($storagePath) / 1073741824, 2),
            ],
            'root' => [
                'free' => round(disk_free_space($rootPath) / 1073741824, 2),
                'total' => round(disk_total_space($rootPath) / 1073741824, 2),
            ],
        ];
        $usedPercent = 100 - (($checks['disk']['root']['free'] / max($checks['disk']['root']['total'], 1)) * 100);
        if ($usedPercent > 90) {
        ...
        // PHP
        $requiredExtensions = ['pdo', 'mbstring', 'openssl', 'json', 'curl', 'gd', 'xml', 'zip', 'fileinfo', 'tokenizer', 'bcmath'];
        ...
        $checks['php'] = [
            ...
            'missing_extensions' => array_values($missingExtensions),
        ];
        ...
        foreach ($dirs as $label => $path) {
            ...
            $checks['permissions']['directories'][$label] = $writable;
            if (!$writable) {
        ...
        // Redis / Cache
        try {
            cache()->put('system_check_test', 'ok', 10);
            $val = cache()->get('system_check_test');
        ...
        try {
            ...
            if ($slowLogEnabled && strtolower($slowLogEnabled->Value) === 'on') {
                $slowQueries = DB::select("SELECT * FROM mysql.slow_log ORDER BY start_time DESC LIMIT 5");
                $checks['slow_queries'] = ['status' => count($slowQueries) > 0 ? 'warning' : 'ok', 'queries' => $slowQueries];
            } else {
                $checks['slow_queries'] = ['status' => 'ok', 'message' => 'Slow query log is disabled'];
            }
        } catch (\Exception $e) {
            $checks['slow_queries'] = ['status' => 'ok', 'message' => 'Cannot access slow query log'];
        }
        ...
        try {
            ...
            // Check cron.log for recent scheduler output
            if (file_exists($cronLog) && filesize($cronLog) > 0) {
                $lines = array_filter(array_map('trim', array_slice(file($cronLog), -20)));
                foreach (array_reverse($lines) as $line) {
                    if (preg_match('/^(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2})/', $line, $m)) {
                        $lastRunAt = Carbon::parse($m[1]);
                        $lastEntry = $line;
                        break;
                    }
                }
            ...
            // Fallback: check scheduler lock file existence (means cron is configured)
            $lockExists = file_exists('/tmp/laravel-scheduler.lock');

            if ($lastRunAt) {
                $diff = now()->diffInMinutes($lastRunAt);
                $checks['cron'] = [
                    'status' => $diff <= 10 ? 'ok' : ($diff <= 30 ? 'warning' : 'error'),
                    'last_run' => $lastRunAt->toDateTimeString(),
                    'minutes_ago' => $diff,
                    'last_entry' => \Illuminate\Support\Str::limit($lastEntry, 120),
                ];
            } elseif ($lockExists) {
                $checks['cron'] = [
                    'status' => 'ok',
                    'message' => 'Scheduler lock file present — cron is configured',
                ];
            } else {
                // Last resort: check laravel.log
                $laravelLog = storage_path('logs/laravel.log');
                $found = false;
                if (file_exists($laravelLog)) {
                    foreach (array_reverse(array_slice(file($laravelLog), -200)) as $line) {
                        if (str_contains($line, 'schedule:run') || str_contains($line, 'Running scheduled')) {
                            $found = true;
                            break;
                        }
                    }
                ...
                $checks['cron'] = [
                    ...
                    'message' => $found ? 'Scheduler activity found in logs' : 'No scheduler activity detected',
                ];
            }
        } catch (\Exception $e) {
            $checks['cron'] = ['status' => 'warning', 'message' => 'Unable to determine cron status'];
        }
        ...
        try {
            ...
            if ($client) {
                ...
                $checks['ssl'] = [
                    'status' => $daysLeft > 30 ? 'ok' : ($daysLeft > 7 ? 'warning' : 'error'),
                ...
                $checks['ssl'] = ['status' => 'warning', 'message' => 'Could not connect to check SSL'];
            }
        } catch (\Exception $e) {
            $checks['ssl'] = ['status' => 'warning', 'message' => 'SSL check unavailable'];
        }

        return view('admin-views.business-settings.system-check', compact('checks'));
    }

    public function systemCheckDiagnose(Request $request)
    {
        Helpers::module_permission_check('settings');
        ...
        // Gather live diagnostic info per check type
        $diagnosticInfo = $this->gatherDiagnosticInfo($check);

        $prompt = "You are a server systems administrator. Analyze this health check data from a Laravel application and provide a concise diagnosis with actionable fix commands.\n\n";
        $prompt .= "Check: {$check}\n";
        $prompt .= "Status Data: " . json_encode($checkData, JSON_PRETTY_PRINT) . "\n";
        $prompt .= "Live Diagnostic Output:\n" . $diagnosticInfo . "\n\n";
        $prompt .= "Respond with:\n1. **Status**: One-line summary\n2. **Issues Found**: List any problems\n3. **Fix Commands**: Ready-to-run shell commands to fix each issue\n4. **Prevention**: One tip to prevent this in future\n\nKeep it short and actionable. Use markdown formatting.";

        try {
            // Use Claude CLI via sudo (authenticated as root)
            $claudeBin = '/usr/local/bin/claude';
            if (!file_exists($claudeBin)) {
                return response()->json(['error' => 'Claude CLI not found at ' . $claudeBin], 500);
            }

            $escapedPrompt = escapeshellarg($prompt);
            $result = shell_exec("sudo {$claudeBin} -p {$escapedPrompt} 2>&1");

            if ($result === null || trim($result) === '') {
                return response()->json(['error' => 'Claude CLI returned no output. Check that the CLI is authenticated.'], 500);
            }

            return response()->json(['diagnosis' => trim($result)]);
        } catch (\Exception $e) {
            return response()->json(['error' => 'Failed to run Claude CLI: ' . $e->getMessage()], 500);
        }
    }

    private function gatherDiagnosticInfo(string $check): string
    {
        $output = '';
        switch ($check) {
            case 'mysql':
                try {
                    $processCount = DB::selectOne("SELECT COUNT(*) as cnt FROM information_schema.processlist")->cnt;
                    $slowVars = DB::selectOne("SHOW VARIABLES LIKE 'slow_query_log'");
                    $waitTimeout = DB::selectOne("SHOW VARIABLES LIKE 'wait_timeout'");
                    $output .= "Active processes: {$processCount}\n";
                    $output .= "Slow query log: " . ($slowVars->Value ?? 'N/A') . "\n";
                    $output .= "Wait timeout: " . ($waitTimeout->Value ?? 'N/A') . "\n";
                    $tableStatus = DB::select("SHOW TABLE STATUS WHERE Engine IS NOT NULL LIMIT 10");
                    foreach ($tableStatus as $t) {
                        if (($t->Data_free ?? 0) > 1048576) {
                            $output .= "Fragmented table: {$t->Name} (free: " . round($t->Data_free / 1048576, 1) . "MB)\n";
                        }
                    }
                } catch (\Exception $e) {
            ...
            case 'disk':
                ...
                $output .= "\nLargest storage dirs:\n";
                $output .= shell_exec('du -sh ' . storage_path() . '/* 2>&1 | sort -rh | head -10') ?? '';
                break;

            case 'php':
                $output .= "PHP Version: " . PHP_VERSION . "\n";
                $output .= "Loaded extensions: " . implode(', ', get_loaded_extensions()) . "\n";
                $output .= "OPcache: " . (function_exists('opcache_get_status') ? json_encode(@opcache_get_status(false)) : 'disabled') . "\n";
                $output .= shell_exec('php -m 2>&1') ?? '';
                break;

            case 'laravel':
                $output .= "ENV: " . config('app.env') . "\n";
                $output .= "Debug: " . (config('app.debug') ? 'ON' : 'OFF') . "\n";
                $output .= "Cache: " . config('cache.default') . "\n";
                $output .= "Queue: " . config('queue.default') . "\n";
                $output .= "Session: " . config('session.driver') . "\n";
                $output .= shell_exec('cd ' . base_path() . ' && php artisan --version 2>&1') ?? '';
                $output .= "\nRoute cache: " . (file_exists(base_path('bootstrap/cache/routes-v7.php')) ? 'cached' : 'not cached') . "\n";
                $output .= "Config cache: " . (file_exists(base_path('bootstrap/cache/config.php')) ? 'cached' : 'not cached') . "\n";
                break;

            case 'permissions':
                $dirs = [storage_path('logs'), storage_path('framework/cache'), storage_path('framework/sessions'), storage_path('framework/views'), storage_path('app/public'), base_path('bootstrap/cache')];
                foreach ($dirs as $dir) {
                    ...
                    $output .= "{$dir}: perms={$perms} owner={$owner} writable=" . (is_writable($dir) ? 'yes' : 'NO') . "\n";
                }
                break;

            case 'mail':
                $output .= "Driver: " . config('mail.default') . "\n";
                ...
                if ($host && $port) {
                    ...
                    if ($conn) {
                        $output .= "SMTP port test: CONNECTED\n";
                        fclose($conn);
                    } else {
                        $output .= "SMTP port test: FAILED ({$errstr})\n";
                    }
                }
                break;

            case 'cache':
                $output .= "Driver: " . config('cache.default') . "\n";
                if (config('cache.default') === 'redis') {
                    $output .= shell_exec('redis-cli ping 2>&1') ?? 'redis-cli not available';
                    $output .= "\n" . (shell_exec('redis-cli info memory 2>&1 | head -5') ?? '');
                }
                try {
                    $start = microtime(true);
                    cache()->put('_diag_test', 'v', 5);
                    cache()->get('_diag_test');
                    cache()->forget('_diag_test');
                    $ms = round((microtime(true) - $start) * 1000, 2);
                    $output .= "\nCache round-trip: {$ms}ms\n";
                } catch (\Exception $e) {
            ...
            case 'slow_queries':
                try {
                    ...
                    $output .= "Long query time threshold: " . ($longQueryTime->Value ?? 'N/A') . "s\n";
                    $slowLogFile = DB::selectOne("SHOW VARIABLES LIKE 'slow_query_log_file'");
                    $output .= "Log file: " . ($slowLogFile->Value ?? 'N/A') . "\n";
                } catch (\Exception $e) {
            ...
            case 'cron':
                $output .= "Crontab:\n" . (shell_exec('crontab -l -u www-data 2>&1') ?? shell_exec('crontab -l 2>&1') ?? 'Cannot read crontab') . "\n";
                $cronLog = storage_path('logs/cron.log');
                if (file_exists($cronLog)) {
                    $output .= "Last 10 cron.log entries:\n";
                    $lines = array_slice(file($cronLog), -10);
                    $output .= implode('', $lines);
                } else {
                    $output .= "cron.log not found\n";
                }
                $output .= "\nScheduler lock: " . (file_exists('/tmp/laravel-scheduler.lock') ? 'exists' : 'not found') . "\n";
                break;

            case 'ssl':
                $host = parse_url(config('app.url'), PHP_URL_HOST);
                if ($host) {
                    $output .= shell_exec("echo | openssl s_client -connect {$host}:443 -servername {$host} 2>&1 | openssl x509 -noout -dates -issuer -subject 2>&1") ?? '';
                } else {
                    $output .= "Cannot determine host from APP_URL\n";
                }
                break;
