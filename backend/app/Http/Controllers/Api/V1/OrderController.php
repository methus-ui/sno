...
public function place_order(Request $request)
{
    ...
    $schedule_at = $request->schedule_at ? \Illuminate\Support\Carbon::parse($request->schedule_at) : now();
    $store = null;
    ...
    // Get delivery fee adjustment using the service (checks time-based schedules first, then static setting)
    if ($zone) {
        $feeAdjustment = $deliveryFeeService->getDeliveryFeeAdjustment($zone, $schedule_at);
        $increased = $feeAdjustment['percentage'];
        $increase_delivery_charge_message = $feeAdjustment['message'];
    }
    ...
    // Apply delivery fee adjustment (supports both increase and decrease)
    if ($increased != 0) {
        $delivery_charge = $deliveryFeeService->applyAdjustment($delivery_charge, $increased);
        $original_delivery_charge = $deliveryFeeService->applyAdjustment($original_delivery_charge, $increased);
    }
    ...
    //Added service charge (with tiered support)
    $additional_charge_status = BusinessSetting::where('key', 'additional_charge_status')->first()->value;
    if ($additional_charge_status == 1) {
        // Use tiered calculation based on order amount
        $order->additional_charge = \App\CentralLogics\Helpers::calculate_tiered_additional_charge($order->order_amount);
    } else {
    ...
    // ✅ FIXED: Multi-cart support - only fetch items from the current store
    $carts = Cart::where('user_id', $order->user_id)
        ->where('is_guest',$order->is_guest)
        ->where('module_id',$request->header('moduleId'))
        ->when(isset($request->store_id) && $request->store_id, function ($query) use ($request) {
            // ✅ CRITICAL FIX: Filter by store to only get items from this store
    ...
    if ($request->order_type !== 'parcel') {
        foreach ($carts as $c) {
            // Convert array to object for buy_now items
            if (is_array($c)) {
                $c = (object) $c;
            }
            ...
            if (isset($c->item_type) && ($c->item_type === 'App\Models\ItemCampaign' || $c->item_type === 'AppModelsItemCampaign')) {
                ...
                if ($product) {
                    ...
                    // ✅ Store validation removed - already filtered by whereHas above
                    ...
                    if ($product->module->module_type == 'food' && $product->food_variations) {
                        ...
                        $quantity = is_array($c) ? $c['quantity'] : $c->quantity;

                        $or_d = [
                            'item_id' => null,
                            'item_campaign_id' => $item_id,
    ...
    try {
        ...
        if(!isset($request->is_buy_now) || (isset($request->is_buy_now) && $request->is_buy_now == 0 )){
            if(isset($request->store_id) && $request->store_id) {
                ...
                // Fallback: delete all carts (for parcel orders or backward compatibility)
                Cart::where('user_id', $order->user_id)
                    ->where('is_guest', $order->is_guest)
                    ->where('module_id', $request->header('moduleId'))
                    ->delete();
        ...
        // ⚠️ NON-BREAKING: Link to bargaining if applicable (completely optional, doesn't affect normal orders)
        if ($request->user) {
            try {
                ...
                // Log error but don't fail order creation - order is already committed
    ...
    public function get_running_orders(Request $request)
    {
        ...
        ->withCount('details')->where(['user_id' => $user_id])->whereNotIn('order_status', ['delivered', 'canceled', 'refund_requested', 'refund_request_canceled', 'refunded', 'failed'])->Notpos()->NotDigitalOrder()->latest()->paginate($request['limit'], ['*'], 'page', $request['offset']);
...
//        })
    ...
    public function cancel_order(Request $request)
    {
        ...
        if (!$order) {
            ...
            if ($order->delivery_man_id) {
                ...
                if ($compensation > 0) {
                    // Send push notification to DM about compensation
                    try {
                        $dm = $order->delivery_man;
                        if ($dm && $dm->fcm_token) {
                            Helpers::send_push_notif_to_device($dm->fcm_token, [
                                'title' => translate('messages.compensation_received'),
                                'description' => translate('messages.you_received_cancellation_compensation') . ' ' . \App\CentralLogics\Helpers::format_currency($compensation),
                                'order_id' => $order->id,
                                'type' => 'order_cancellation_compensation'
                            ]);
                        }
                    } catch (\Exception $e) {
                        \Log::error("Failed to send DM compensation notification: " . $e->getMessage());
                    }
                }
            }
    ...
    public function order_again(Request $request){
        ...
        if (empty($zone_id)) {
            return response()->json([
                'errors' => [
                    ['code' => 'zone_id', 'message' => translate('messages.zone_id_required')]
                ]
            ], 403);
        }

        $stores = Store::withOpen($longitude,$latitude)->
        ...
        // Load top 5 items per store using window function (efficient for large catalogs)
        $storeIds = $stores->pluck('id')->toArray();
        if (!empty($storeIds)) {
            $placeholders = implode(',', array_fill(0, count($storeIds), '?'));
            $topItems = \DB::select("
                SELECT * FROM (
                    SELECT id, name, price, stock, image, category_id, store_id, status,
                           avg_rating, recommended, discount, discount_type, description,
                           is_approved, unit_id, module_id, slug, order_count, rating_count,
                           ROW_NUMBER() OVER (PARTITION BY store_id ORDER BY avg_rating DESC, recommended DESC) AS rn
                    FROM items
                    WHERE store_id IN ({$placeholders})
                ) ranked
                WHERE rn <= 5
            ", $storeIds);
            $itemsByStore = collect($topItems)->groupBy('store_id');
        } else {
            $itemsByStore = collect([]);
        }

        $data = $stores->map(function ($store) use ($itemsByStore) {
            $store->items = $itemsByStore->get($store->id, collect());
            return $store;
        });
...
public function reorderOrder(Request $request)
{
    ...
    $order = Order::with(['details.item', 'store'])
    ...
    if (!$order->store) {
        return response()->json([
            'errors' => [
                ['code' => 'store', 'message' => translate('messages.store_not_available')]
            ]
        ], 404);
    }
    ...
    $schedule_at = now();
    $store = Store::selectRaw('*, IF(((select count(*) from `store_schedule` where `stores`.`id` = `store_schedule`.`store_id` and `store_schedule`.`day` = ' . $schedule_at->format('w') . ' and `store_schedule`.`opening_time` < "' . $schedule_at->format('H:i:s') . '" and `store_schedule`.`closing_time` >"' . $schedule_at->format('H:i:s') . '") > 0), true, false) as open')
    ...
    if (!$store || !$store->open) {
        return response()->json([
            'errors' => [
                ['code' => 'store', 'message' => translate('messages.store_is_closed')]
            ]
        ], 403);
    }
    ...
    try {
        ...
        foreach ($order->details as $detail) {
            ...
            if (!$item || !$item->status) {
                $unavailable_items[] = $detail->item_details ? json_decode($detail->item_details)->name : 'Unknown Item';
                continue;
            }

            // Check stock for non-food items
            if ($item->module->module_type != 'food') {
                $variation = json_decode($detail->variation, true);
                if (count(json_decode($item->variations, true)) > 0 && count($variation) > 0) {
                    $variant_data = Helpers::variation_price($item, json_encode($variation));
                    $stock = $variant_data['stock'];
                } else {
                    $stock = $item->stock;
                }

                if (config('module.' . $item->module->module_type)['stock'] && $detail->quantity > $stock) {
                    $unavailable_items[] = $item->name . ' (Out of stock)';
                    continue;
                }
            }

            // Check if item already in cart from the same store
            $existing_cart = Cart::where('user_id', $request->user->id)
                ->where('item_id', $item->id)
                ->where('variation', $detail->variation)
                ->first();

            if ($existing_cart) {
                // Update quantity
                $existing_cart->quantity += $detail->quantity;
                $existing_cart->save();
                $added_items[] = $item->name . ' (quantity updated)';
            } else {
                ...
                $cart->variation = $detail->variation;
                $cart->add_on_ids = $detail->add_ons ? json_encode(array_column(json_decode($detail->add_ons, true), 'id')) : json_encode([]);
                $cart->add_on_qtys = $detail->add_ons ? json_encode(array_column(json_decode($detail->add_ons, true), 'quantity')) : json_encode([]);
                $cart->module_id = $order->module_id;
                $cart->is_guest = 0;
                $cart->save();
        ...
        if (empty($added_items) && !empty($unavailable_items)) {
            return response()->json([
                'errors' => [
                    ['code' => 'items', 'message' => translate('messages.all_items_unavailable')]
                ]
            ], 403);
        }

        return response()->json([
            'message' => translate('messages.items_added_to_cart_successfully'),
            'added_items' => $added_items,
            'unavailable_items' => $unavailable_items,
            'cart_count' => Cart::where('user_id', $request->user->id)
                ->whereHas('item', function($q) use($order) {
        ...
        return response()->json([
            'errors' => [
                ['code' => 'reorder', 'message' => translate('messages.failed_to_add_items')]
            ]
        ], 403);
    }
}
    ...
    /**
     * Get order details with bargaining data (Customer App API)
     *
     * @param Request $request
     * @param int $orderId
     * @return JsonResponse
     */
    public function getOrderDetailsWithBargaining(Request $request, $orderId)
    {
        $order = Order::with([
            'details.item',
            'details.campaign',
        ...
        if (!$order) {
            return response()->json([
                'errors' => [['code' => 'order', 'message' => translate('messages.order_not_found')]]
            ], 404);
        }

        // Verify user owns this order (or is guest with matching guest_id)
        if ($request->user()) {
            if ($order->user_id !== $request->user()->id) {
                return response()->json([
                    'errors' => [['code' => 'auth', 'message' => translate('messages.unauthorized')]]
                ], 403);
            }
        } elseif ($request->has('guest_id')) {
            if ($order->guest_id !== $request->guest_id || !$order->is_guest) {
                return response()->json([
                    'errors' => [['code' => 'auth', 'message' => translate('messages.unauthorized')]]
                ], 403);
            }
        ...
        $response = [
            ...
            'scheduled_at' => $order->schedule_at ? $order->schedule_at->toDateTimeString() : null,
            'is_bargaining_order' => (bool) $order->is_bargaining_order,
            ...
            'customer' => $order->customer ? [
                ...
                'image' => $order->customer->image_full_url,
            ] : null,
            ...
            'store' => [
                ...
                'rating' => (float) ($order->store->rating ?? 0),
            ...
            'delivery_man' => $order->delivery_man ? [
                ...
                'image' => $order->delivery_man->image_full_url,
            ] : null,
            ...
            'items' => $order->details->map(function($detail) use ($order) {
                return [
                    ...
                    'original_price' => $order->is_bargaining_order && isset($detail->original_price) ? (float) $detail->original_price : null,
                    'savings_per_item' => $order->is_bargaining_order && isset($detail->original_price) ? (float) ($detail->original_price - $detail->price) : null,
            ...
            'order_summary' => [
                'subtotal' => (float) ($order->order_amount - $order->total_tax_amount - $order->delivery_charge - ($order->dm_tips ?? 0)),
                'bargaining_discount' => 0.0,
                'store_discount' => (float) ($order->store_discount_amount ?? 0),
                'coupon_discount' => (float) ($order->coupon_discount_amount ?? 0),
                'tax' => (float) $order->total_tax_amount,
                'delivery_fee' => (float) $order->delivery_charge,
                'dm_tips' => (float) ($order->dm_tips ?? 0),
        ...
        if ($order->is_bargaining_order && $order->bargainingRequest) {
            $response['bargaining_data'] = [
                ...
                'original_cart_value' => (float) $order->bargainingRequest->original_cart_value,
                'final_price' => (float) $order->bargainingRequest->final_price,
                'total_savings' => (float) $order->bargainingRequest->total_savings,
                'savings_percentage' => round(($order->bargainingRequest->total_savings / $order->bargainingRequest->original_cart_value) * 100, 1),
                'total_offers_received' => (int) $order->bargainingRequest->total_offers_received,
                'winning_store' => [
                    'id' => $order->store->id,
                    'name' => $order->store->name,
                    'logo' => $order->store->logo_full_url,
                ...
                'fulfillment_percentage' => (float) ($order->bargainingAcceptedOffer?->fulfillment_percentage ?? 100),
                'items_missing' => (int) ($order->bargainingAcceptedOffer?->items_missing ?? 0),
                ...
                'time_to_accept' => $order->bargainingRequest->accepted_at ?
                    $order->bargainingRequest->created_at->diffInSeconds($order->bargainingRequest->accepted_at) : null,
                'competing_offers' => isset($order->bargainingRequest->storeOffers) ? $order->bargainingRequest->storeOffers->map(function($offer) use ($order) {
                    return [
                        'rank' => (int) $offer->rank,
                        ...
                        'items_available' => (int) $offer->items_available,
                        'items_missing' => (int) $offer->items_missing,
                        'is_winner' => $offer->id === $order->bargaining_accepted_offer_id,
                    ];
                }) : [],
            ];

            $response['order_summary']['bargaining_discount'] = -(float) $order->bargainingRequest->total_savings;
        }
