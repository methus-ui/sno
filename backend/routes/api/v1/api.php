...
Route::group(['namespace' => 'Api\V1', 'middleware'=>'localization'], function () {
    ...
    Route::group(['prefix' => 'auth', 'namespace' => 'Auth'], function () {
        ...
        Route::group(['prefix' => 'delivery-man'], function () {
            Route::post('send-registration-otp', 'DeliveryManLoginController@sendRegistrationOtp');
            Route::post('verify-registration-otp', 'DeliveryManLoginController@verifyRegistrationOtp');
    ...
    Route::group(['prefix' => 'delivery-man'], function () {
        ...
        Route::group(['middleware'=>['dm.api']], function () {
            Route::post('logout', 'Auth\DeliveryManLoginController@logout');
            Route::get('profile', 'DeliverymanController@get_profile');

            // Security Deposit Routes
            Route::group(['prefix' => 'security-deposit'], function () {
                Route::post('pay', 'SecurityDepositController@pay');
                Route::get('status', 'SecurityDepositController@status');
            });

            // Weekly Earnings Route
            Route::get('weekly-earnings', 'DeliverymanController@get_weekly_earnings');
            ...
            Route::post('check-delivery-proximity', 'DeliverymanController@check_delivery_proximity');
            ...
            Route::post('update-item-location', 'DeliverymanController@update_item_location');
            //Remove account
            ...
            Route::post('mark-outside-purchase', 'DeliverymanController@mark_outside_purchase');
            ...
            // Mark item unavailable, upload bill image, request MRP update
            Route::put('mark-item-unavailable', 'DeliverymanController@mark_item_unavailable');
            Route::put('update-item-pickup', 'DeliverymanController@update_item_pickup');
            Route::post('upload-bill-image', 'DeliverymanController@upload_bill_image');
            Route::put('request-mrp-update', 'DeliverymanController@request_mrp_update');

            // Performance & Ranking
            Route::get('performance-stats', 'DeliverymanController@getPerformanceStats');
            Route::get('leaderboard', 'DeliverymanController@getLeaderboard');
            Route::get('incentive-breakdown', 'DeliverymanController@getIncentiveBreakdown');
            Route::get('rush-status', 'DeliverymanController@getRushStatus');

            // Shift Management (DEPRECATED - use Shift Booking System below)
            Route::get('available-shifts', 'DeliverymanController@getAvailableShifts'); // DEPRECATED - use shift-slots
            Route::post('self-assign-shift', 'DeliverymanController@selfAssignShift'); // DEPRECATED - use book-shift
            Route::get('my-shifts', 'DeliverymanController@getMyShifts'); // DEPRECATED - use my-bookings
            Route::post('request-shift-swap', 'DeliverymanController@requestShiftSwap'); // Temporarily disabled

            // Shift Booking System (NEW - Recommended)
            Route::get('shift-slots', 'DeliverymanController@getShiftSlots');
            Route::get('my-bookings', 'DeliverymanController@getMyBookings');
            Route::post('book-shift', 'DeliverymanController@bookShift');
            Route::post('cancel-shift-booking', 'DeliverymanController@cancelShiftBooking');

            // Notification Preferences
            Route::post('update-notification-preferences', 'DeliverymanController@updateNotificationPreferences');

            // Heatmap & Hotspots
            Route::get('heatmap', 'DeliverymanController@getHeatmap');
            Route::get('hotspots', 'DeliverymanController@getHotspots');
            Route::get('active-cart-hotspots', 'DeliverymanController@getActiveCartHotspots');
            Route::get('demand-forecast', 'DeliverymanController@getDemandForecast');

            // Order Batching
            Route::get('current-batch', 'DeliverymanController@getCurrentBatch');

            // SOS Emergency
            Route::post('sos', 'DeliverymanController@triggerSos');
            Route::get('sos/active', 'DeliverymanController@getActiveSos');

            // Referral
            Route::get('referral-code', 'DeliverymanController@getReferralCode');
            Route::get('referral-stats', 'DeliverymanController@getReferralStats');

            // Instant Withdrawal
            Route::post('request-withdrawal', 'DeliverymanController@requestWithdrawal');
            Route::get('withdrawal-history', 'DeliverymanController@getWithdrawalHistory');
    ...
    Route::group(['prefix' => 'vendor', 'namespace' => 'Vendor', 'middleware'=>['vendor.api']], function () {
        ...
        Route::put('order/mark-item-unavailable', 'VendorController@mark_item_unavailable');
       ...
       // advertisement
        ...
        Route::group(['prefix'=>'item'], function(){
            ...
            Route::POST('search', 'ItemController@search');
                     Route::post('toggle-stock-status', 'ItemController@toggle_stock_status');
                     Route::post('quick-stock-update', 'ItemController@quick_stock_update');
    ...
    Route::group(['middleware'=>['module-check']], function(){
        Route::group(['prefix' => 'customer', 'middleware' => 'auth:api'], function () {
            ...
            /**
             * Security Fix (2026-01-28): Replaced withoutMiddleware with external.api middleware
             * This adds rate limiting, IP whitelisting, and audit logging while maintaining functionality.
            ...
            Route::group(['prefix'=>'wallet'], function() {
                ...
                /**
                 * Security Fix (2026-01-28): Added external.api middleware for protection
                 * Rollback: Run storage/security_backups/20260128/ROLLBACK.sh
                 */
                Route::post('transfer-mart-from-drivemond', 'WalletController@transferMartFromDrivemondWallet')
        ...
        Route::group(['prefix' => 'customer', 'middleware' => 'apiGuestCheck'], function () {
            Route::group(['prefix' => 'order'], function () {
                ...
                Route::put('payment-method', 'OrderController@update_payment_method');
                         Route::post('reorder', 'OrderController@reorderOrder');
        ...
        Route::group(['prefix' => 'categories'], function () {
            ...
            Route::get('/{category_id}/products', 'CategoryController@get_category_items');
...
// Temporary bargaining status route
Route::middleware(['auth:api'])->group(function () {
    Route::get('bargaining/status/{code}', function($code) {
...
/*
|--------------------------------------------------------------------------
| V2 API Routes - Bargaining Mode
|--------------------------------------------------------------------------
*/
// Bargaining routes moved to routes/api/v2/api.php to avoid duplicate loading
// DO NOT load here - it's already loaded via RouteServiceProvider
/*
Route::group(['prefix' => 'v2', 'namespace' => 'Api\V2', 'middleware' => 'localization'], function () {
    require __DIR__ . '/../v2/bargaining.php';
});
*/

WebSocketsRouter::webSocket('/delivery-man/live-location', DMLocationSocketHandler::class);

// Temporary bargaining status route (while v2 routes are being fixed)
Route::middleware(['auth:api'])->prefix('bargaining')->group(function () {
    Route::get('status/{code}', function($code) {
        $request = \App\Models\BargainingRequest::where('request_code', $code)->first();
        
        if (!$request || $request->user_id != auth()->id()) {
            return response()->json(['errors' => [['code' => 'not_found', 'message' => 'Request not found']]], 404);
        }
        ...
        return response()->json([
            ...
            'original_cart_value' => (float) $request->original_cart_value,
            'total_stores_matched' => $request->total_stores_matched,
            'total_offers_received' => $request->storeOffers->count(),
            'time_remaining' => $request->time_remaining,
            'offers' => $offers,
        ]);
    });
