...
<head>
    ...
    <!-- Cache Control: Force fresh load on every visit -->
...
@include("layouts.admin.partials._sidebar" . ($module_type ? "_{$module_type}" : ""))
...
<!-- End Footer -->
    ...
    {{-- Bill Uploaded Notification Modal --}}
    <div class="modal fade" id="bill-uploaded-modal">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-body">
                    <div class="text-center">
                        <h2>
                            <i class="tio-receipt"></i> {{translate('messages.bill_received')}}
                        </h2>
                        <p id="bill-uploaded-description" class="mt-2 text-muted"></p>
                        <hr>
                        <a id="bill-uploaded-link" href="#" class="btn btn-primary">{{translate('messages.view_order')}}</a>
                        <button type="button" class="btn btn-secondary ml-2" data-dismiss="modal">{{translate('messages.close')}}</button>
                    </div>
                </div>
            </div>
        </div>
                        ...
                        <table class="table table-borderless table-sm">
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td>{{ translate('Navigate to All Orders (Current Module)') }}</td>
                                </tr>
                                <tr>
                                    <td><kbd class="bg-dark text-white px-2 py-1 rounded">O</kbd></td>
                                    <td>{{ translate('Navigate to Grocery Orders') }}</td>
                                </tr>
                                <tr>
                                    <td><kbd class="bg-dark text-white px-2 py-1 rounded">P</kbd></td>
                                    <td>{{ translate('Navigate to Parcel Orders') }}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
...
"use strict";
    ...
    function startFCM() {
        ...
        messaging
            ...
            // Silently handle FCM errors (usually invalid/missing Firebase credentials)
            console.log('FCM not available:', error.code || 'Configuration issue');
        });
    }
    ...
    // OLD CHAT FUNCTIONS - REMOVED (Replaced by React Chat)
    // Functions removed: conversationList, conversationView, vendorConversationView, dmConversationView
    // Date removed: 2026-03-12
    ...
    messaging.onMessage(function(payload) {
        ...
        if(payload.data.order_id && payload.data.type == "bill_uploaded"){
                playAudio();
                $('#bill-uploaded-description').text(payload.data.body || 'Bill uploaded for order #' + payload.data.order_id);
                $('#bill-uploaded-link').attr('href', '{{url("/")}}/admin/order/details/' + payload.data.order_id);
                $('#bill-uploaded-modal').appendTo("body").modal('show');
                toastr.info(payload.data.body || 'Bill received for order #' + payload.data.order_id, payload.data.title || 'Bill Received', {
                    CloseButton: true,
                    ProgressBar: true,
                    timeOut: 10000
                });
        } else if(payload.data.order_id && payload.data.type == "order_request"){
            ...
            // OLD CHAT MESSAGE HANDLER - REMOVED (Replaced by React Chat Widget)
            // The floating chat widget now handles new message notifications
        }
    });
    ...
    // Poll for new bill uploads every 15 seconds
    setInterval(function () {
        $.get({
            url: '{{route('admin.check-new-bills')}}',
            dataType: 'json',
            success: function (response) {
                if (response.count > 0) {
                    response.bills.forEach(function(bill) {
                        playAudio();
                        var msg = 'Bill uploaded for order #' + bill.order_id + ' (' + bill.store_name + ')';
                        $('#bill-uploaded-description').text(msg);
                        $('#bill-uploaded-link').attr('href', '{{url("/")}}/admin/order/details/' + bill.order_id);
                        $('#bill-uploaded-modal').appendTo("body").modal('show');
                        toastr.info(msg, '{{ translate("messages.bill_received") }}', {
                            CloseButton: true,
                            ProgressBar: true,
                            timeOut: 10000
                        });
    ...
    // OLD CHAT INITIALIZATION - DISABLED (Replaced by React Chat Widget)
    // conversationList();
...
@include('layouts.admin.partials._message-notifications')
    ...
    document.addEventListener('keydown', function(event) {
        ...
        if (!isTyping) {
            ...
            if (event.key === 'k' || event.key === 'K') {
                ...
                window.location.href = '{{ route('admin.order.list', ['all']) }}';
            }

            // O - Navigate to Grocery Orders (module_id = 2)
            if (event.key === 'o' || event.key === 'O') {
                event.preventDefault();
                window.location.href = '{{ url('/') }}/admin/order/list/all?module_id=2';
            }

            // P - Navigate to Parcel Orders (module_id = 3)
            if (event.key === 'p' || event.key === 'P') {
                event.preventDefault();
                window.location.href = '{{ url('/') }}/admin/parcel/orders/all?module_id=3';
            }
        }
    });
</script>
