...
<body>
    <div class="dashboard-container">
        ...
        <div class="todays-section" style="margin-top: 50px;">
            <h2 class="section-title">🔍 Filters & Analytics</h2>

            <div class="filter-toolbar" style="background: linear-gradient(135deg, #1a1a1a 0%, #252525 100%); border: 1px solid var(--border); border-radius: 16px; padding: 25px; margin-bottom: 30px;">
                <div class="row g-3">
                    <div class="col-md-3">
                        <label for="filter-zone" style="color: var(--text-secondary); font-size: 14px; font-weight: 600; margin-bottom: 8px; display: block;">
                            <i class="fas fa-map-marker-alt" style="color: var(--primary);"></i> Zone
                        </label>
                        <select id="filter-zone" class="form-select filter-control" style="background: var(--bg-card); border: 1px solid var(--border); color: var(--text-primary); padding: 10px 15px; border-radius: 8px;">
                    ...
                    <div class="col-md-3">
                        <label for="filter-date-range" style="color: var(--text-secondary); font-size: 14px; font-weight: 600; margin-bottom: 8px; display: block;">
                            <i class="fas fa-calendar-alt" style="color: var(--primary);"></i> Date Range
                        </label>
                        <select id="filter-date-range" class="form-select filter-control" style="background: var(--bg-card); border: 1px solid var(--border); color: var(--text-primary); padding: 10px 15px; border-radius: 8px;">
                    ...
                    <div class="col-md-3" id="custom-date-container" style="display: none;">
                        <label for="filter-custom-date" style="color: var(--text-secondary); font-size: 14px; font-weight: 600; margin-bottom: 8px; display: block;">
                            <i class="fas fa-calendar-day" style="color: var(--primary);"></i> Custom Date
                        </label>
                        <input type="date" id="filter-custom-date" class="form-control filter-control" style="background: var(--bg-card); border: 1px solid var(--border); color: var(--text-primary); padding: 10px 15px; border-radius: 8px;">
                    </div>

                    <div class="col-md-3">
                        <label for="filter-status" style="color: var(--text-secondary); font-size: 14px; font-weight: 600; margin-bottom: 8px; display: block;">
                            <i class="fas fa-filter" style="color: var(--primary);"></i> Order Status
                        </label>
                        <select id="filter-status" class="form-select filter-control" style="background: var(--bg-card); border: 1px solid var(--border); color: var(--text-primary); padding: 10px 15px; border-radius: 8px;">
                    ...
                    <div class="col-md-12 text-end" style="margin-top: 15px;">
                        <button id="filter-reset" class="btn" style="background: var(--gradient-primary); color: white; padding: 10px 25px; border-radius: 8px; font-weight: 600; border: none; cursor: pointer; transition: var(--transition);">
                            <i class="fas fa-redo-alt"></i> Reset Filters
                        </button>
                        <span id="filter-loading" style="display: none; margin-left: 15px; color: var(--primary); font-weight: 600;">
        ...
        <div class="todays-section">
            <h2 class="section-title">📊 Hourly Order Distribution</h2>
            <div class="chart-container modern-chart-card">
                <div id="hourly-chart" style="min-height: 400px;"></div>
                <p class="chart-description">
                    <i class="fas fa-info-circle"></i> Peak ordering hours help optimize staff scheduling and delivery fleet allocation
                </p>
            </div>
        </div>
        ...
        <div class="todays-section">
            <h2 class="section-title">⏱️ Delivery Time Distribution</h2>
            <div class="chart-container modern-chart-card">
                <div id="delivery-time-chart" style="min-height: 400px;"></div>
                <div class="chart-legend">
                    <div class="legend-item">
                        <span class="legend-dot" style="background: #10b981;"></span>
                        <span>Express (&lt;20min)</span>
                    </div>
                    <div class="legend-item">
                        <span class="legend-dot" style="background: #84cc16;"></span>
                        <span>Fast (20-30min)</span>
                    </div>
                    <div class="legend-item">
                        <span class="legend-dot" style="background: #eab308;"></span>
                        <span>Standard (30-50min)</span>
                    </div>
                    <div class="legend-item">
                        <span class="legend-dot" style="background: #ef4444;"></span>
                        <span>Slow (&gt;50min)</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="todays-section">
            ...
            <div class="chart-container modern-chart-card">
                <div id="status-chart" style="min-height: 450px;"></div>
                <p class="chart-description">
                    <i class="fas fa-mouse-pointer"></i> Real-time order pipeline visibility
                </p>
            </div>
        </div>
        ...
        <!-- 90-DAY TREND CHART -->
        <div class="todays-section">
            <h2 class="section-title">📈 90-Day Historical Trends</h2>
            <div class="chart-container modern-chart-card">
                <div id="trend-chart" style="min-height: 500px;"></div>
                <p class="chart-description">
                    <i class="fas fa-search-plus"></i> Use the toolbar to zoom, pan, and toggle data series
                </p>
            </div>
        </div>
    ...
    <!-- Include Delivery Stats Charts Module -->
    <script src="{{ asset('public/assets/admin/js/delivery-stats-charts.js') }}?v={{ time() }}"></script>

    <!-- Include Delivery Stats Filters Module -->
    <script src="{{ asset('public/assets/admin/js/delivery-stats-filters.js') }}?v={{ time() }}"></script>

    <!-- Embedded Chart Data (No AJAX needed) -->
    <script>
        console.log('🟢 Embedded chart data loaded');

        // Make chart data available globally
        window.INITIAL_CHART_DATA = {
            hourly: @json($chartData['hourly'] ?? []),
            status: @json($chartData['status'] ?? []),
            trend: @json($chartData['trend'] ?? []),
            deliveryTime: @json($chartData['deliveryTime'] ?? [])
        };

        console.log('📊 Chart data available:', {
            hourly: window.INITIAL_CHART_DATA.hourly.length + ' hours',
            status: window.INITIAL_CHART_DATA.status.total + ' total orders',
            trend: window.INITIAL_CHART_DATA.trend.length + ' days',
            deliveryTime: window.INITIAL_CHART_DATA.deliveryTime.buckets.length + ' buckets'
        });
    </script>

    <!-- Modern Dark UI Chart Styling -->
    <style>
        /* Modern Chart Card */
        .modern-chart-card {
            background: linear-gradient(135deg, #1a1a1a 0%, #252525 100%);
            border: 1px solid rgba(216, 39, 107, 0.2);
            border-radius: 20px;
            padding: 35px;
            margin-bottom: 40px;
        ...
        .modern-chart-card::before {
            ...
            right: 0;
            height: 4px;
            background: linear-gradient(90deg, #D8276B 0%, #E8477B 100%);
        }

        .modern-chart-card:hover {
            transform: translateY(-4px);
            box-shadow: 0 12px 48px rgba(216, 39, 107, 0.2);
            border-color: rgba(216, 39, 107, 0.4);
        }
        ...
        .filter-control:focus {
            outline: none;
            border-color: #D8276B !important;
            box-shadow: 0 0 0 4px rgba(216, 39, 107, 0.15);
        }
        ...
        .apexcharts-tooltip {
            ...
            border: 1px solid rgba(216, 39, 107, 0.3) !important;
            box-shadow: 0 4px 16px rgba(0, 0, 0, 0.4) !important;
        }

        .apexcharts-tooltip-title {
            background: linear-gradient(135deg, #D8276B 0%, #E8477B 100%) !important;
