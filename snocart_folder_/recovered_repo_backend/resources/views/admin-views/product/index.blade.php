                        ...
                        <div class="card-body d-flex flex-wrap align-items-center">
                            <div class="w-100 d-flex flex-wrap __gap-15px">
                                ...
                                <div class="flex-grow-1 mx-auto">
                                    ...
                                    <div class="mt-2 d-none" id="thumbnail-edit-wrap">
                                        <a href="javascript:void(0)" class="btn btn-sm btn-warning" id="edit-thumbnail-btn" title="{{ translate('Edit Image') }}">
...
@push('script_2')
    ...
    <div class="col-xl-4 col-lg-6">
        ...
        // Show edit button when thumbnail is selected
        $('#customFileEg1').on('change', function() {
            if (this.files && this.files[0]) {
                $('#thumbnail-edit-wrap').removeClass('d-none');
            }
        });

        // Edit thumbnail (new image)
        $('#edit-thumbnail-btn').on('click', function() {
            var viewerSrc = $('#viewer').attr('src');
            if (!viewerSrc || viewerSrc.indexOf('upload-img') !== -1) {
                toastr.error('{{ translate("Please select an image first") }}');
                return;
            }
            openImageEditor(viewerSrc, {
                sourceType: 'new',
                fileInput: document.getElementById('customFileEg1'),
                imgElement: document.getElementById('viewer'),
                csrfToken: '{{ csrf_token() }}'
            });
        });

        // Inject edit button on spartan gallery previews
        $(document).on('click', '.spartan_item_wrapper .edit-gallery-new-btn', function() {
            ...
            if (!imgSrc || imgSrc.indexOf('upload-img') !== -1) return;
