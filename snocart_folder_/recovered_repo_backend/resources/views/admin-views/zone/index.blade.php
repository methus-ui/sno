    ...
    <div class="content container-fluid">
        ...
        data-original-title="{{translate('messages.draw_your_zone_on_the_map')}}">{{translate('messages.draw_your_zone_on_the_map')}}</span></label>
                                ...
                                <div class="map-warper rounded mt-0" style="position: relative; height: 500px;">
                                    ...
                                    <div id="map-canvas" class="rounded" style="height: 100%; width: 100%;"></div>
                                </div>
                            </div>
                        </div>
                    </div>
...
@push('script_2')
    <script async defer src="https://maps.googleapis.com/maps/api/js?key={{\App\Models\BusinessSetting::where('key', 'map_api_key')->first()->value}}&callback=initialize&libraries=drawing,places,geometry&v=3.49"></script>
<script>
    "use strict";
    let pointsDrawn = 0;
    let allZonePolygons = [];
    ...
    function auto_grow() {
        ...
        function checkForOverlaps(newPolygon) {
            ...
            allZonePolygons.forEach((existingPolygon, index) => {
                ...
                if (hasOverlap) {
                    overlappingZones.push(index + 1);
                    overlapDetected = true;
                }
            });

            if (overlapDetected) {
                $('#overlap-message').text(`Zone overlaps with Zone ${overlappingZones.join(', ')}`);
