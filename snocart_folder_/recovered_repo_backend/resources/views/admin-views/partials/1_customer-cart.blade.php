@php $index = 1; @endphp
@foreach($latest_carts as $userId => $userCarts)
    @php
        $customer = $users->get($userId);
        if (!$customer) continue;
        
        $totalItems = $userCarts->sum('quantity');
        $cartTotal = $userCarts->sum(function($cart) {
            return ($cart->price ?? 0) * ($cart->quantity ?? 0);
        });
        $cartAge = \Carbon\Carbon::parse($userCarts->first()->created_at);
        $minutesAgo = $cartAge->diffInMinutes(now());
        
        // Get unique stores from cart items
        $storeIds = $userCarts->pluck('store_id')->unique()->filter();
        $multipleStores = $storeIds->count() > 1;
        
        // Get primary store name for top display
        $primaryStoreId = $storeIds->first();
        $primaryStoreName = isset($stores) && $stores->has($primaryStoreId)
            ? $stores->get($primaryStoreId)->name
            : null;
    @endphp
    
    <!-- Compact Cart Card -->
    <div class="col mb-2 cart-card-item" data-index="{{$index}}" style="{{$index > 8 ? 'display: none;' : ''}}">
        <div class="card h-100 cart-card-modern">
            
            <!-- TOP: Store Name Badge (NEW) -->
            @if($primaryStoreName)
                <div class="card-store-banner">
                    <div class="d-flex align-items-center justify-content-between">
                        <div class="d-flex align-items-center flex-grow-1 min-width-0">
                            <i class="tio-store text-white mr-2"></i>
                            <span class="text-white font-weight-semibold text-truncate">
                                {{$primaryStoreName}}
                            </span>
                        </div>
                        @if($multipleStores)
                            <span class="badge badge-light badge-sm ml-2">
                                +{{$storeIds->count() - 1}} more
                            </span>
                        @endif
                    </div>
                </div>
            @elseif($multipleStores)
                <div class="card-store-banner bg-warning">
                    <div class="d-flex align-items-center">
                        <i class="tio-store text-white mr-2"></i>
                        <span class="text-white font-weight-semibold">
                            {{$storeIds->count()}} Stores
                        </span>
                    </div>
                </div>
            @endif
            
            <!-- Customer Header -->
            <div class="card-header bg-white border-bottom-0 pb-0 pt-3">
                <div class="d-flex align-items-center">
                    <div class="avatar avatar-sm avatar-circle mr-2">
                        <img class="avatar-img"
                             onerror="this.src='{{asset('public/assets/admin/img/160x160/img1.jpg')}}'"
                             src="{{$customer->image_full_url ?? asset('public/assets/admin/img/160x160/img1.jpg')}}"
                             alt="{{$customer->f_name}}">
                    </div>
                    <div class="flex-grow-1 min-width-0">
                        @if(Route::has('admin.customer.view'))
                            <a href="{{route('admin.customer.view', ['user_id' => $customer->id])}}"
                               class="d-block text-dark font-weight-semibold text-truncate">
                                {{$customer->f_name}} {{$customer->l_name}}
                            </a>
                        @else
                            <div class="font-weight-semibold text-truncate">
                                {{$customer->f_name}} {{$customer->l_name}}
                            </div>
                        @endif
                        <small class="text-muted d-block text-truncate">
                            <i class="tio-call-outlined"></i> {{$customer->phone ?? 'N/A'}}
                        </small>
                    </div>
                </div>
            </div>

            <!-- Items Section -->
            <div class="card-body py-2 px-3">
                <div class="cart-items-compact">
                    @foreach($userCarts->take(3) as $cartItem)
                        @php
                            $item = $items->get($cartItem->item_id);
                            $itemStoreName = isset($stores) && isset($cartItem->store_id) && $stores->has($cartItem->store_id)
                                ? $stores->get($cartItem->store_id)->name
                                : null;
                        @endphp
                        @if($item)
                            <div class="mb-2">
                                <div class="d-flex justify-content-between align-items-start mb-1">
                                    <div class="text-truncate flex-grow-1 pr-2">
                                        <span class="badge badge-primary badge-sm">{{$cartItem->quantity}}x</span>
                                        <small class="ml-1">{{Str::limit($item->name ?? 'Item', 20)}}</small>
                                    </div>
                                    <small class="text-muted text-nowrap">
                                        {{\App\CentralLogics\Helpers::format_currency($cartItem->price * $cartItem->quantity)}}
                                    </small>
                                </div>
                                @if($multipleStores && $itemStoreName)
                                    <div class="pl-4">
                                        <small class="text-muted">
                                            <i class="tio-store-outlined" style="font-size: 10px;"></i>
                                            {{Str::limit($itemStoreName, 18)}}
                                        </small>
                                    </div>
                                @endif
                            </div>
                        @endif
                    @endforeach
                    @if($userCarts->count() > 3)
                        <a href="#" class="text-primary small"
                           onclick="showAllItems(event, '{{$customer->f_name}} {{$customer->l_name}}', {{$userId}})">
                            <i class="tio-add"></i> {{$userCarts->count() - 3}} more items
                        </a>
                    @endif
                </div>
            </div>

            <!-- Footer Summary -->
            <div class="card-footer bg-light py-2 px-3">
                <div class="d-flex justify-content-between align-items-center mb-1">
                    <span class="badge badge-soft-info badge-sm">
                        <i class="tio-shopping-basket-outlined"></i> {{$totalItems}} items
                    </span>
                    @if($minutesAgo < 5)
                        <span class="badge badge-danger badge-sm">
                            <i class="tio-time"></i> {{$minutesAgo}}m ago
                        </span>
                    @elseif($minutesAgo < 30)
                        <span class="badge badge-warning badge-sm">
                            <i class="tio-time"></i> {{$minutesAgo}}m ago
                        </span>
                    @elseif($minutesAgo < 1440)
                        <span class="badge badge-info badge-sm">
                            <i class="tio-time"></i> {{$cartAge->diffInHours(now())}}h ago
                        </span>
                    @else
                        <span class="badge badge-secondary badge-sm">
                            <i class="tio-time"></i> {{$cartAge->diffInDays(now())}}d ago
                        </span>
                    @endif
                </div>
                <div class="d-flex justify-content-between align-items-center">
                    <span class="text-primary font-weight-bold">
                        {{\App\CentralLogics\Helpers::format_currency($cartTotal)}}
                    </span>
                    @if($multipleStores)
                        <small class="text-warning">
                            <i class="tio-info-outlined"></i> Multi-store
                        </small>
                    @endif
                </div>
            </div>
        </div>
    </div>
    
    @php $index++; @endphp
@endforeach
