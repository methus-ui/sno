                                        ...
                                        <form id="search-form" class="search-form">
                                            ...
                                            <div class="position-relative">
                                                ...
                                                <span class="search-icon">
                                                    <img width="16" height="16" src="{{asset('public/assets/admin/img/icons/search-icon.png')}}" alt="">
                                                </span>

                                                @if($keyword)
                                                    <button type="button" class="btn btn--primary ml-2 location-reload-to-base-pos" data-url="{{url()->full()}}">{{translate('messages.reset')}}</button>
                                                @endif
                                            </div>
                                            <!-- End Search -->
                                        </form>
...
@push('script_2')
{{-- Add POS Search System - MUST BE FIRST --}}
<link rel="stylesheet" href="{{ asset('public/assets/admin/css/pos-search.css') }}">
...
<script>
    ...
    // REMOVED - OLD SEARCH FORM HANDLER IS COMMENTED OUT
    // The new pos-search.js handles search automatically
    /*
...
(function() {
    'use strict';
    console.log('🔍 POS AJAX Search Loading...');
    ...
    setTimeout(function() {
        ...
        // STEP 1: Remove all jQuery handlers
        $('#search-form').off();
        $(searchInput).off();
        console.log('✅ jQuery handlers removed');
        
        // STEP 2: Clone to remove native listeners
        const newForm = searchForm.cloneNode(true);
        searchForm.parentNode.replaceChild(newForm, searchForm);
        const newInput = document.getElementById('datatableSearch');
        console.log('✅ Form and input cloned (native handlers removed)');
        ...
        function performSearch(keyword) {
            ...
            if (!storeId) {
                console.warn('⚠️  No store selected - cannot search');
                return;
            }
            ...
            console.log('🔎 Searching for:', trimmed || '(showing all)');
            
            productGrid.style.opacity = '0.6';
            productGrid.style.pointerEvents = 'none';
            
            const searchUrl = '/admin/pos/search-products?keyword=' + encodeURIComponent(trimmed) +
                            '&store_id=' + storeId +
            ...
            .then(data => {
                if (data.success) {
                    productGrid.innerHTML = data.view;
                    console.log('✅ Search complete:', data.count, 'products found');
        ...
        // STEP 3: Add input event (handles typing AND backspace)
        ...
        // STEP 4: Handle search clear button (X)
        newInput.addEventListener('search', function(e) {
            console.log('🧹 Search cleared');
            clearTimeout(searchTimeout);
            performSearch('');
        }, true);
        
        // STEP 5: Block form submission
        ...
        document.addEventListener('keydown', function(e) {
            ...
            if ((e.ctrlKey || e.metaKey) && e.key === 'k') {
                ...
                newInput.focus();
                newInput.select();
                console.log('⌨️  Search focused via Ctrl+K');
            }
            ...
            if (e.key === 'Escape' && document.activeElement === newInput) {
                newInput.value = '';
                performSearch('');
                console.log('⌨️  Search cleared via Escape');
            }
        }, true);
        
        console.log('✅ ✅ ✅ AJAX Search System Ready! ✅ ✅ ✅');
...
#datatableSearch:focus::placeholder {
    color: #9CA3AF;
}
</style>
@endpush
