...
class Helpers
{
    ...
    {
        // Only reject null values - allow both arrays and Model objects
        if ($data === null) {
            return null;
        }
        $variations = [];
        $categories = [];
        $category_ids = gettype($data['category_ids'] ?? null) == 'array' ? $data['category_ids'] : json_decode($data['category_ids'] ?? '[]',true);
        if (is_array($category_ids)) {
            foreach ($category_ids as $value) {
                // Handle both formats: [5, 7] (integers) or [{'id':5, 'position':1}] (objects)
                if (is_array($value)) {
                    ...
                    // Just an integer category ID
                    $category_name = Category::where('id',$value)->pluck('name');
                    $categories[] = ['id' => (string)$value, 'position' => 0, 'name'=>data_get($category_name,'0','NA')];
                }
            }
        }
    ...
    public static function product_data_formatting($data, $multi_data = false, $trans = false, $local = 'en' , $temp_product=false)
    {
        $storage = [];
        if ($multi_data == true) {
            // PERFORMANCE FIX: Pre-load all required data before loop to avoid N+1 queries
            $allCategoryIds = [];
            $allAddonIds = [];
            $allItemIds = [];
            ...
            $categoryNames = !empty($allCategoryIds) ? Category::whereIn('id', $allCategoryIds)->pluck('name', 'id') : collect();

            // Batch load addons
            $allAddonIds = array_unique(array_filter($allAddonIds));
            $allAddons = !empty($allAddonIds) ? AddOn::whereIn('id', $allAddonIds)->active()->get()->keyBy('id') : collect();

            // Batch load flash sales
            $flashSaleItems = FlashSaleItem::Active()
            ...
            foreach ($data as $item) {
                ...
                // PERFORMANCE FIX: Use pre-loaded category names instead of querying in loop
                $categories = [];
                foreach (json_decode($item['category_ids']) ?? [] as $key => $value) {
                    // Handle both formats: simple integers [1,2,3] and objects [{"id":1,"position":1}]
                    if (is_object($value)) {
                        $categories[] = ['id' => (string)$value->id, 'position' => $value->position, 'name' => $categoryNames->get($value->id, 'NA')];
                    } else {
                        // Handle simple integer array format
                        $categories[] = ['id' => (string)$value, 'position' => $key + 1, 'name' => $categoryNames->get($value, 'NA')];
                    }
                }
                $item['category_ids'] = $categories;
                $item['attributes'] = json_decode($item['attributes']);
                $item['choice_options'] = json_decode($item['choice_options']);
                // PERFORMANCE FIX: Use pre-loaded addons instead of querying in loop
                $itemAddonIds = json_decode($item['add_ons'], true) ?? [];
                $itemAddons = $allAddons->only($itemAddonIds)->values();
                ...
                // PERFORMANCE FIX: Use pre-loaded flash sale data instead of querying in loop
                $running_flash_sale = $flashSaleItems->get($item['id']);
                ...
                // PERFORMANCE FIX: Use already-loaded relations instead of querying again
            ...
            $variations = [];
            $categories = [];
            foreach (json_decode($data['category_ids']) ?? [] as $key => $value) {
                // Handle both formats: simple integers [1,2,3] and objects [{"id":1,"position":1}]
                if (is_object($value)) {
                    $category_name = Category::where('id',$value->id)->pluck('name');
                    $categories[] = ['id' => (string)$value->id, 'position' => $value->position, 'name'=>data_get($category_name,'0','NA')];
                    ...
                    // Handle simple integer array format
                    $category_name = Category::where('id',$value)->pluck('name');
                    $categories[] = ['id' => (string)$value, 'position' => $key + 1, 'name'=>data_get($category_name,'0','NA')];
                }
            }
            $data['category_ids'] = $categories;
    ...
    public static function order_data_formatting($data, $multi_data = false)
    {
        ...
        if ($multi_data) {
            ...
            // Add order edit information for delivery person
            $data['is_edited'] = (bool)($data['edited'] ?? false);
            $data['adjustment_amount'] = (float)($data['adjustment_amount'] ?? 0);
            $data['cod_collection_amount'] = (float)($data['cod_collection_amount'] ?? 0);

            // Calculate total to collect from customer (for display)
            $totalToCollect = 0;
            if ($data['payment_method'] == 'cash_on_delivery') {
                // For pure COD, total to collect = order_amount
                $totalToCollect = (float)($data['order_amount'] ?? 0);
            } elseif ($data['payment_method'] == 'partial_payment') {
                // For partial payment, find unpaid COD amount
                if (isset($data['payments']) && is_object($data['payments'])) {
                    foreach ($data['payments'] as $payment) {
                        if ($payment->payment_method == 'cash_on_delivery' && $payment->payment_status == 'unpaid') {
                            $totalToCollect += (float)$payment->amount;
                        }
                    }
                }
            } else {
                // For wallet/digital payments, only collect adjustment amount if any
                $totalToCollect = (float)($data['cod_collection_amount'] ?? 0);
            }
            $data['total_to_collect'] = round($totalToCollect, 2);
    ...
    public static function order_details_data_formatting($data)
    {
        ...
        foreach ($data as $item) {
            ...
            // Include discount-adjusted price for delivery app display
            $qty = max((int)$item['quantity'], 1);
            $discountPerUnit = (float)($item['discount_on_item'] ?? 0) / $qty;
            $item['original_price'] = (float)$item['price'];
            $item['discount_per_unit'] = round($discountPerUnit, 2);
            $item['discounted_price'] = round((float)$item['price'] - $discountPerUnit, 2);
            $item['total_after_discount'] = round($item['discounted_price'] * $qty, 2);
            // Override price so delivery app shows after-discount amount
    ...
    public static function deliverymen_list_formatting($data, $storeCoords = null, $currentOrderCustomerCoords = null)
    {
        $storage = [];
        foreach ($data as $item) {
            $storage_type = 'public';
            ...
            $avg_rating = 0;
            $rating_count = 0;
            if ($item->relationLoaded('rating') && count($item->rating) > 0) {
                $avg_rating = (float) $item->rating[0]->average;
                $rating_count = (int) $item->rating[0]->rating_count;
            }

            // Get total delivered orders count
            $total_delivered = $item->relationLoaded('total_delivered_orders') ? $item->total_delivered_orders->count() : 0;

            // Get total ongoing orders count
            $total_ongoing = $item->relationLoaded('total_ongoing_orders') ? $item->total_ongoing_orders->count() : 0;

            // Get cash in hand
            $cash_in_hand = ($item->relationLoaded('wallet') && $item->wallet) ? $item->wallet->collected_cash : 0;

            // Get vehicle info
            $vehicle_type = ($item->relationLoaded('vehicle') && $item->vehicle) ? $item->vehicle->type : null;

            // Get zone info
            $zone_id = $item['zone_id'] ?? null;
            $zone_name = ($item->relationLoaded('zone') && $item->zone) ? $item->zone->name : null;

            // Check if DM is available (under max orders limit)
            $max_orders = config('dm_maximum_orders') ?? 1;
            $is_available = ($item['current_orders'] ?? 0) < $max_orders;

            // Calculate distance from store if coordinates provided
            $distance = null;
            $distance_text = 'Unknown';
            $distance_color = 'secondary';
            if ($storeCoords && $item->last_location) {
            ...
            // Build active orders list with same-route detection
            $active_orders = [];
            $has_same_route_order = false;
            if ($item->relationLoaded('orders')) {
                $sameRouteThresholdKm = 2; // orders within 2km considered same route
                foreach ($item->orders as $activeOrder) {
                    $orderInfo = [
                        'id' => $activeOrder->id,
                        'status' => $activeOrder->order_status,
                        'store_name' => $activeOrder->store->name ?? 'N/A',
                        'same_route' => false,
                    ];

                    // Check if this active order's store is near current order's store
                    $nearStore = false;
                    if ($storeCoords && $activeOrder->store && $activeOrder->store->latitude && $activeOrder->store->longitude) {
                        $storeToStoreDist = self::calculate_haversine_distance(
                            $storeCoords['lat'], $storeCoords['lng'],
                            (float)$activeOrder->store->latitude, (float)$activeOrder->store->longitude
                        );
                        if ($storeToStoreDist !== null && $storeToStoreDist <= $sameRouteThresholdKm) {
                            $nearStore = true;
                        }
                    }

                    // Check if this active order's delivery address is near current order's customer
                    $nearCustomer = false;
                    if ($currentOrderCustomerCoords && !empty($activeOrder->delivery_address)) {
                        $delAddr = is_string($activeOrder->delivery_address) ? json_decode($activeOrder->delivery_address, true) : $activeOrder->delivery_address;
                        if (!empty($delAddr['latitude']) && !empty($delAddr['longitude'])) {
                            $custToCustDist = self::calculate_haversine_distance(
                                $currentOrderCustomerCoords['lat'], $currentOrderCustomerCoords['lng'],
                                (float)$delAddr['latitude'], (float)$delAddr['longitude']
                            );
                            if ($custToCustDist !== null && $custToCustDist <= $sameRouteThresholdKm) {
                                $nearCustomer = true;
                            }
                        }
                    }

                    if ($nearStore && $nearCustomer) {
                        $orderInfo['same_route'] = true;
                        $has_same_route_order = true;
                    }
            ...
            $storage[] = [
                ...
                'current_orders' => $item['current_orders'] ?? 0,
                ...
                'has_same_route_order' => $has_same_route_order,
            ];
        }
        $data = $storage;
    ...
    /**
     * Get Bootstrap color class based on distance
     */
    public static function get_distance_color($distance)
    {
        if ($distance === null) return 'secondary';
        if ($distance < 2) return 'success';      // < 2km = green
        if ($distance < 5) return 'info';         // 2-5km = blue
        if ($distance < 10) return 'warning';     // 5-10km = yellow/orange
        return 'danger';                          // > 10km = red
    }
...
//        }
    ...
    /**
     * Update image - uploads new image and deletes old one only if not used by other items
     *
     * @param string $dir Directory path (e.g., 'product/')
     * @param string $old_image Old image filename to potentially delete
     * @param string $format Image format (e.g., 'png')
     * @param mixed $image New image file
     * @param int|null $excludeItemId Item ID to exclude from usage check
     * @param int|null $excludeTempProductId TempProduct ID to exclude from usage check
     * @return string New image filename or old image if no new image provided
     */
    public static function update(string $dir, $old_image, string $format, $image = null, ?int $excludeItemId = null, ?int $excludeTempProductId = null)
    {
        if ($image == null) {
            return $old_image;
        }
        ...
        // Only delete old image if it's not used by other items
        if ($old_image && $old_image !== 'def.png' && !self::isImageUsedByOtherItems($old_image, $excludeItemId, $excludeTempProductId)) {
            try {
    ...
    /**
     * Check if an image is used by other items (different item IDs)
     * This prevents deletion of images that are shared across items (e.g., cloned items)
     *
     * @param string $imageName The image filename to check
     * @param int|null $excludeItemId Item ID to exclude from the check (the item being modified)
     * @param int|null $excludeTempProductId TempProduct ID to exclude from the check
     * @return bool True if image is used by other items, false otherwise
     */
    public static function isImageUsedByOtherItems(string $imageName, ?int $excludeItemId = null, ?int $excludeTempProductId = null): bool
    {
        if (empty($imageName) || $imageName === 'def.png') {
            return false;
        }

        try {
            // Check if any other Item uses this image as main image
            $itemMainImageCount = Item::withoutGlobalScopes()
            ...
            if ($itemMainImageCount > 0) {
                return true;
            }

            // Check if any other Item uses this image in gallery images
            $itemsWithGallery = Item::withoutGlobalScopes()
                ->whereNotNull('images')
                ->when($excludeItemId, function ($query) use ($excludeItemId) {
                    $query->where('id', '!=', $excludeItemId);
                })
            ...
            // Check TempProduct main image
            $tempMainImageCount = TempProduct::withoutGlobalScopes()
                ->where('image', $imageName)
                ->when($excludeTempProductId, function ($query) use ($excludeTempProductId) {
                    $query->where('id', '!=', $excludeTempProductId);
                })
                ->count();

            if ($tempMainImageCount > 0) {
                return true;
            }

            // Check TempProduct gallery images
            $tempProductsWithGallery = TempProduct::withoutGlobalScopes()
                ->whereNotNull('images')
                ->when($excludeTempProductId, function ($query) use ($excludeTempProductId) {
                    $query->where('id', '!=', $excludeTempProductId);
                })
            ...
            foreach ($tempProductsWithGallery as $tempProduct) {
                $images = is_array($tempProduct->images) ? $tempProduct->images : json_decode($tempProduct->images, true);
                if (is_array($images)) {
                    foreach ($images as $img) {
                        $imgName = is_array($img) ? ($img['img'] ?? null) : $img;
                        if ($imgName === $imageName) {
            ...
            // Check ItemCampaign if exists
            if (class_exists(ItemCampaign::class)) {
                $campaignCount = ItemCampaign::withoutGlobalScopes()
                    ->where('image', $imageName)
                    ->count();
                if ($campaignCount > 0) {
                    return true;
            ...
            // If any error occurs, return true to prevent accidental deletion
            return true;
        }
    ...
    /**
     * Safely delete an image file only if no other items are using it
     *
     * @param string $dir Directory path (e.g., 'product/')
     * @param string $old_image Image filename to delete
     * @param int|null $excludeItemId Item ID to exclude from usage check
     * @param int|null $excludeTempProductId TempProduct ID to exclude from usage check
     * @return bool True if deleted or skipped, false on error
     */
    public static function check_and_delete(string $dir, $old_image, ?int $excludeItemId = null, ?int $excludeTempProductId = null)
    {
        if (empty($old_image) || $old_image === 'def.png') {
            return true;
        }

        // Check if image is used by other items before deleting
        if (self::isImageUsedByOtherItems($old_image, $excludeItemId, $excludeTempProductId)) {
            // Image is shared with other items, don't delete the file
            return true;
        }
    ...
    public static function get_language_name($key)
    {
        ...
        {
            return \Cache::rememberForever('view_keys', function () {
                ...
                return $keys->map(fn($v) => (bool)$v)->toArray();
            });
        }
...
//                return rtrim($awsUrl, '/') . '/' . ltrim($awsBucket . '/' . $path . '/' . $data, '/');
    ...
    public static function subscription_plan_chosen($store_id ,$package_id, $payment_method  ,$discount = 0,$pending_bill =0,$reference=null ,$type=null){
        ...
        try {
            ...
            // Auto-approve ALL stores immediately - no manual approval needed
            $store->status = 1;
            $store_subscription->status = 1;

            // Also ensure vendor is approved
            if ($store->vendor && $store->vendor->status != 1) {
    ...
    /**
     * Calculate tiered additional charge based on order amount
     *
     * @param float $order_amount The total order amount (before additional charge)
     * @return float The calculated additional charge
     */
    public static function calculate_tiered_additional_charge($order_amount)
    {
        // IMPORTANT: This function should only be called when additional_charge_status is enabled
        // The OrderController checks this before calling this function
        ...
        if (!$tiered_enabled || $tiered_enabled->value != '1') {
            // Fall back to flat charge if tiered system is disabled
            $additional_charge = BusinessSetting::where('key', 'additional_charge')->first();
            return $additional_charge ? (float)$additional_charge->value : 0;
        }

        // Get tier configuration
        $tiers_setting = BusinessSetting::where('key', 'additional_charge_tiers')->first();

        if (!$tiers_setting || !$tiers_setting->value) {
            // No tiers configured, fall back to flat charge
            $additional_charge = BusinessSetting::where('key', 'additional_charge')->first();
            return $additional_charge ? (float)$additional_charge->value : 0;
        }

        // Parse tiers JSON
        $tiers = json_decode($tiers_setting->value, true);

        if (!is_array($tiers) || empty($tiers)) {
            // Invalid tiers data, fall back to flat charge
            $additional_charge = BusinessSetting::where('key', 'additional_charge')->first();
            return $additional_charge ? (float)$additional_charge->value : 0;
        }
        ...
        foreach ($tiers as $tier) {
            ...
            $charge = isset($tier['charge']) ? (float)$tier['charge'] : 0;

            // Check if order amount falls within this tier
            if ($order_amount >= $min) {
                if ($max === null || $order_amount <= $max) {
                    return $charge;
                }
            }
        }
