...
// Security Deposit Payment Hooks
if (! function_exists('security_deposit_fail')) {
    function security_deposit_fail($data){
        try {
            if ($data->attribute === 'security_deposit_payment') {
                $payment = \App\Models\SecurityDepositPayment::find($data->attribute_id);
                if ($payment) {
            ...
            info('Security deposit fail hook error: ' . $exception->getMessage());
        }
        return 0;
    }
}

if (! function_exists('security_deposit_success')) {
        ...
        try {
            if ($data->attribute === 'security_deposit_payment') {
                ...
                if ($payment) {
                    ...
                    $payment->paid_at = now();
                    $payment->transaction_id = $data->transaction_ref ?? $data->payment_method . '_' . time();
                    $payment->save();

                    // Update delivery man status
                    $deliveryMan = $payment->deliveryMan;
                    if ($deliveryMan) {
                        $deliveryMan->security_deposit_status = 'paid';
                        $deliveryMan->security_deposit_paid_at = now();
                        $deliveryMan->security_deposit_transaction_id = $payment->transaction_id;
                        $deliveryMan->security_deposit_payment_method = $data->payment_method;
                        $deliveryMan->save();
                    }
                    ...
                    try {
                        if (config('mail.status') && $deliveryMan && $deliveryMan->email) {
                            // You can create a SecurityDepositMail class if needed
                            // Mail::to($deliveryMan->email)->send(new \App\Mail\SecurityDepositMail($payment, $deliveryMan->f_name));
                        }
                    } catch (\Exception $e) {
                        info('Security deposit email error: ' . $e->getMessage());
                    }
                }
            }
        } catch (\Exception $exception) {
            info('Security deposit success hook error: ' . $exception->getMessage());
            DB::rollBack();
        }
        return true;
    }
