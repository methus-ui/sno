...
class CategoryController extends Controller
{
    ...
    // ✅ ONLY THIS METHOD IS UPDATED - adds representative images
public function get_childes($id, Request $request)
{
    try {
        ...
        // ✅ Add representative images to each subcategory
        foreach ($categories as $category) {
            // ✅ Get random product image with transparent background (PNG files)
            $randomItem = Item::active()
                ->where('category_id', $category->id)
                ->when(!empty($zone_id), function($q) use ($zone_id) {
                    $q->whereHas('store', fn($sq) => $sq->whereIn('zone_id', $zone_id));
                })
                ->whereNotNull('image')
                ->where('image', '!=', '')
                // ✅ Prioritize PNG images (transparent background support)
            ...
            $category->representative_image_url = $randomItem
                ? asset('storage/app/public/product/' . $randomItem->image)
                : null;

            // ✅ Add accurate product count for zone filtering
            if (!empty($zone_id)) {
                $category->products_count = Item::active()
                    ->where('category_id', $category->id)
                    ->whereHas('store', fn($q) => $q->whereIn('zone_id', $zone_id))
                    ->count();
            } else {
                $category->products_count = Item::active()
                    ->where('category_id', $category->id)
        ...
        // ✅ Remove subcategories with no products
        ...
        \Log::error('Error in get_childes: ' . $e->getMessage());
        return response()->json([], 200);
    }
}
...
public function get_category_products(Request $request)
{
    ...
    $data = CategoryLogic::category_products($category_ids, $zone_id, $limit, $offset, $type);
    $data['products'] = Helpers::product_data_formatting($data['products'], true, false, app()->getLocale());
