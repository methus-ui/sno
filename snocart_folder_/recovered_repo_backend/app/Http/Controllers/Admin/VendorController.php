...
public function getCartData(Request $request)
{
    try {
        ...
        $storeId = auth('vendor')->user()->stores[0]->id ?? null;
        ...
        // Get latest carts (last 50 active carts)
        $latest_carts = \App\Models\Cart::with(['item'])
            ->whereHas('item', function($query) use ($storeId) {
                $query->where('store_id', $storeId);
            })
            ->where('created_at', '>=', now()->subHours(24)) // Only carts from last 24 hours
