...
class NotificationController extends BaseController
{
    ...
    private function getAddView($request): View
        {
            ...
            // ✅ Always initialize $notification as null
            $notification = null;

            return view(NotificationViewPath::INDEX[VIEW], compact('notifications', 'zones', 'notification'));
        }
        ...
        public function add(NotificationAddRequest $request): JsonResponse
        {
            ...
            $notification->image = $notification->image
                ? $notification->toArray()['image_full_url']
    ...
    private function updateCartActivity($userId)
    {
        DB::table('carts')
            ->where('user_id', $userId)
            ->update([
                'last_activity_at' => now(),
                'abandoned_notification_sent' => false // Reset flag on activity
            ]);
    }
