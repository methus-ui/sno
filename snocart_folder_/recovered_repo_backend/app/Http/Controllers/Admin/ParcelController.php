...
class ParcelController extends Controller
{
    ...
    public function order_details(Request $request, $id)
    {
        ...
        if (isset($order)) {
            $inZoneDeliveryMen = DeliveryMan::withOutGlobalScope(ZoneScope::class)->where('zone_id',$order->zone_id)->where(function($query)use($order){
                $query->where('vehicle_id',$order->dm_vehicle_id)->orWhereNull('vehicle_id');
            })->available()->active()->get();

            $outOfZoneDeliveryMen = DeliveryMan::withOutGlobalScope(ZoneScope::class)->where('zone_id','!=',$order->zone_id)->available()->active()->get();
            ...
            $storeCoords = null;
            $inZoneDeliveryMen = Helpers::deliverymen_list_formatting($inZoneDeliveryMen, $storeCoords);
            $inZoneDeliveryMen = collect($inZoneDeliveryMen)->sortBy('distance')->values()->all();
            $outOfZoneDeliveryMen = Helpers::deliverymen_list_formatting($outOfZoneDeliveryMen, $storeCoords);
            ...
            $keyword = null;
            return view('admin-views.order.order-view', compact('order', 'deliveryMen', 'inZoneDeliveryMen', 'outOfZoneDeliveryMen', 'zones', 'storeCoords', 'categories', 'products', 'category', 'keyword', 'editing'));
        } else {
