...
class Store extends Model
{
    ...
    /**
     * @return HasMany
     * @deprecated For large datasets, use itemsOptimized() instead to avoid slow queries
     * WARNING: This loads all 40 columns. On stores with many items, this can cause 2-3 second delays.
     */
    ...
    /**
     * Optimized items relationship - selects only essential columns
     * Use this for list views to reduce memory usage and query time
     * @return HasMany
     */
    public function itemsOptimized(): HasMany
    {
        return $this->hasMany(Item::class)
            ->select(['id', 'name', 'price', 'stock', 'image', 'category_id', 'store_id', 'status', 'avg_rating', 'recommended', 'discount', 'discount_type']);
    }
     ...
     * @return HasOne
     */
    public function bargainingSetting(): HasOne
    {
        return $this->hasOne(StoreBargainingSetting::class);
    }
    ...
    {
        // Validate and sanitize coordinates to prevent SQL syntax errors
        $longitude = is_numeric($longitude) && $longitude != 0 ? floatval($longitude) : 0;
        $latitude = is_numeric($latitude) && $latitude != 0 ? floatval($latitude) : 0;
